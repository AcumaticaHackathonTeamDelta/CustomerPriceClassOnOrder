	...
	public class PXView
	{
        ...
        private bool CorrectCacheAndField(string originField, out PXCache cache, out string field)
        {
            ...
            if (idx > -1)
            {
                ...
                PXCache joinedTableCache = tables
                                           ...
                                           .Select(t => _Graph.Caches[t.Name]).FirstOrDefault();
		...
		protected internal bool prepareFilters(ref PXFilterRow[] filters, string forceView = null)
		{
			...
			if (filters != null)
			{
				...
				{
					var state = cache.GetStateExt(null, f.DataField) as PXStringState;
...
//			if (SelectQueries.IsCommandMutable && queryKey.CommandText == null && queryKey.BadParamsQueryNotExecuted) 
		...
		protected virtual void SortResult(List<object> list, PXSearchColumn[] sorts, bool reverseOrder)
		{
			...
			for (int i = 0; i < sorts.Length; i++)
			{
				...
				if (!reverseOrder)
				{
					comparisons[i] = (a, b) => CompareMethod(a, b, cache, fieldName, @descending, useExt, tableType, collationCmp, dict);
				}
				else
				{
					comparisons[i] = (a, b) => -CompareMethod(a, b, cache, fieldName, @descending, useExt, tableType, collationCmp, dict);
			...
			)
		{
			...
			for (int i = 0; i < filters.Length; i++)
			{
				...
				var state = cache.GetStateExt(null, fr.DataField) as PXStringState;
				if (state != null && state.MultiSelect)
			...
			)
		{
			...
			for (int k = 0; k < sorts.Length; k++)
			{
				if (sorts[k].SearchValue != null)
				{
					...
					{
						return SearchMethod(a, searchValue, cache, fieldName, descending, useExt, tableType, dict);
					});
				}
			}
		...
		public sealed class PXSearchColumn : PXSortColumn
		{
...
//			IBqlParameter[] selpars = _Select.GetParameters();
			...
			if (list == null)
			{
				...
				PXCommandKey currentKey = SelectQueries.Keys.FirstOrDefault(k => k.Equals(queryKey));
				if (!((BqlSelect.Context != null && BqlSelect.Context.BadParametersSkipMergeCache) || (currentKey != null && currentKey.BadParamsSkipMergeCache)))
		...
		protected int CompareMethod(object a, object b, PXCache cache, string fieldName, bool descending, bool useExt, Type tableType, PXCollationComparer collationComparer, Dictionary<string, string> valueLabelDic)
		{
			if (tableType != null)
			{
			...
			if (aVal == null)
				...
				result = compareGuid((Guid)aVal, (Guid)bVal);
			else
		...
		protected int SearchMethod(object a, object bVal, PXCache cache, string fieldName, bool descending, bool useExt, Type tableType, Dictionary<string, string> valueLabelDic)
		{
			if (tableType != null)
			{
				a = ((PXResult)a)[tableType];

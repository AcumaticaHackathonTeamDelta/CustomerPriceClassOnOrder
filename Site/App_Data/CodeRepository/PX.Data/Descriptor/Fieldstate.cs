...
namespace PX.Data
{
	...
	public class PXFieldState : IDataSourceFieldSchema, ICloneable
	{
        ...
        /// only.</summary>
		public virtual bool IsReadOnly { get { return _IsReadOnly || !_Enabled; } }
	...
	public class PXStringState : PXFieldState
	{
		...
		public virtual string[] AllowedImages
		{
			get
			{
				...
				return new string[_AllowedValues.Length];
			}
	...
	public class PXSegmentedState : PXStringState
	{
		...
		private static string[] _headerList;
		...
		public static PXFieldState CreateInstance(object value, string fieldName, PXSegment[] segments, string viewName, bool? validCombos, string wildcard)
		{
			...
			if (String.IsNullOrEmpty(state._ViewName))
			{
				state._FieldList = new string[] { "Value", "Descr" };
				if(_headerList == null)
					_headerList = new [] { PXMessages.LocalizeNoPrefix("Value"), PXMessages.LocalizeNoPrefix("Description") };
				state._HeaderList = _headerList;
                state._ValueField = "Value";
				...
				for (int i = 0; i < segments.Length; i++)
				{
					...
					if (s.Length > 0)
					{
						...
						switch (s.EditMask)
						{
							case 'C':
							case 'c':
								bld.Append(new string('C', s.Length));
								break;
							case 'A':
							case 'a':
								bld.Append(new string('A', s.Length));
								break;
							case '9':
								bld.Append(new string('#', s.Length));
								break;
							case '?':
								bld.Append(new string('L', s.Length));
								break;
						}
					}
	...
	public class PXNoteState : PXFieldState
	{
		...
		public static PXFieldState CreateInstance(object value, string fieldName, string noteIDField, string noteTextField, string noteDocsField, string noteActivityField, string noteTextExistsField = null, string noteFilesExistsField = null)
		{
			PXNoteState state = value as PXNoteState;
			if (state == null)
			{

...
namespace PX.Data
{
	...
	public class PXDBLocalizableStringAttribute : PXDBStringAttribute, IPXRowPersistedSubscriber
	{
		...
		public static string GetTranslation(PXCache sender, object row, string fieldName, string language)
		{
			if (IsEnabled && !String.IsNullOrWhiteSpace(language))
			{
				...
				string[] translations = sender.GetValueExt(row, fieldName + TranslationsPostfix) as string[];
				int idx;
			...
			return sender.GetValue(row, fieldName) as string;
		}
		...
		public static PXDataField GetValueSelect(string tableName, string fieldName, bool isLong)
		{
			...
			if (provider != null)
			{
				...
				{
					restriction = String.Format("({2}.CompanyID IN ({0}, {1}))", listCompanies, cid, extTable);
					multicompany = true;
				}
				else
				{
				...
				if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Select)
				{
					...
					if (description != null)
					{
						...
						if ((e.Operation & PXDBOperation.Option) == PXDBOperation.External)
						{
							...
							{
								var value = ordered[i].ToUpper();
								kvList.Add(String.Format("{0} = {1}", sqlDialect.quoteTableAndColumn(extTable, "FieldName"), sqlDialect.quoteLiteral(_FieldName + value)));
								kvList.Add(i.ToString());
							}
							...
							for (int i = 0; i < ordered.Count; i++)
							{
								bld.Append(sqlDialect.enquoteValue(_FieldName + ordered[i].ToUpper()));
								if (i < ordered.Count - 1)
								{
									bld.Append(", ");
								}
				...
				if (e.TranStatus == PXTranStatus.Open)
				{
					...
					}
					{
						...
						if (description != null && !String.IsNullOrEmpty(description.FieldName))
						{
							...
							PXDataFieldAssign assign = new PXDataFieldAssign("RecordID", description.DataType, description.DataLength, description.DataValue);
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			if (_Definition != null)
			{
				...
				for (int i = 0; i < _Definition.DefaultPlusAlternative.Count; i++)
				{
					...
					_FieldIndexes[_FieldName + _Definition.DefaultPlusAlternative[i].ToUpper()] = i;
				}
				if (NonDB)
				{
    ...
    /// from the <see cref="PXDateAttribute">PXDBDate</see> attribute.
    /// <code>
    ...
    /// cref="PXDBScalarAttribute">PXDBScalar</see>. You can also use BQL
    ...
    /// cref="PXFormulaAttribute">PXFormula</see> attribute.</para>
	...
	[Serializable]
	[PXDBGuid]
	[PXUIField(DisplayName = "Created By", Enabled = false, Visible = true)]
	public class PXDBCreatedByIDAttribute : PXAggregateAttribute, IPXRowInsertingSubscriber, IPXFieldVerifyingSubscriber
	{
		...
		{
			_Attributes.Add(new PXDisplaySelectorAttribute(search, fields) { DescriptionField = descriptionField, SubstituteKey = substituteKey, CacheGlobal = true });
	...
	public class PXDBLastModifiedByIDAttribute : PXDBCreatedByIDAttribute, IPXRowUpdatingSubscriber
	{
				...
				typeof(Modifier.pKID), typeof(Modifier.username))
		{
	        ...
	        {
		        _Attributes.Add(new PXUIFieldAttribute{ Enabled = false, Visible = true});
			...
			uint msgnum;
			string msgprefix;
			uiattr.DisplayName = PXMessages.Localize(DisplayFieldNames.LastModifiedByID, out msgnum, out msgprefix);
		}
	...
	public class PXDBEmailAttribute : PXDBStringAttribute, IPXFieldVerifyingSubscriber
	{
		...
		private static bool IsPossibleEmail(string address)
		{
			if (address == null)
				return false;
			if (address.Length < 5)
			...
			var pos = address.LastIndexOf("@", StringComparison.Ordinal);
			bool isValid = pos > 0 && (address.LastIndexOf(".", StringComparison.Ordinal) > pos) && (address.Length - pos > 2);
			if (!isValid)
				return false;

			//detect html or javascript injection
						...
						if (id != null && Convert.ToInt32(id) > 0) {
							...
							foreach (string field in cache.Fields) {
								...
								if (description != null && !String.IsNullOrEmpty(description.FieldName) && description.IsRestriction && description.DataValue != null && description.DataType != PXDbType.Timestamp) {
									if (typeof(PXCacheExtension).IsAssignableFrom(_BqlTable) && _BqlTable.IsDefined(typeof(PXTableAttribute), true) && cache.Keys.Contains(field))
									{
										string fieldName = sender.Graph.SqlDialect.unquoteColumn(description.FieldName);
										int idx = fieldName.IndexOf('.');
										if (idx >= 0)
										{
											fieldName = fieldName.Substring(idx + 1);
										}
										if (!PXDatabase.Provider.GetTableStructure(_BqlTable.Name).Columns.Any(c => string.Equals(c.Name, field, StringComparison.OrdinalIgnoreCase)))
										{
											continue;
										}
									}

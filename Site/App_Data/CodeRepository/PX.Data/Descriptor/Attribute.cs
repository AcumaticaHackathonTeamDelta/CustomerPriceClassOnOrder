...
namespace PX.Data
{
	...
	public class PXDefaultAttribute : PXEventSubscriberAttribute, IPXFieldDefaultingSubscriber, IPXRowPersistingSubscriber, IPXFieldSelectingSubscriber
	{
		...
		protected static Tuple<object, PXCache> SelectRow(PXCache cache, PXDefaultAttribute attr, object data)
		{
			...
			if (attr._CacheGlobal && attr._ReferencedField != null && (key = cache.GetValue(data, attr._ReferencedField.Name)) != null)
			{
				...
				dict = PXContext.GetSlot<PXSelectorAttribute.GlobalDictionary>(type.FullName);
				if (dict == null)
				{
					PXContext.SetSlot<PXSelectorAttribute.GlobalDictionary>(type.FullName, (dict = PXDatabase.GetSlot<PXSelectorAttribute.GlobalDictionary>(type.FullName, cache.Graph.Caches[type].BqlTable)));
				}
				lock ((dict).SyncRoot)
				{
					KeyValuePair<object, bool> pair;
					if (dict.TryGetValue(key, out pair) && !pair.Value && !(pair.Key is IDictionary))
					{
						row = pair.Key;
					}
				}
	...
	public class PXParentAttribute : PXEventSubscriberAttribute, IPXRowInsertedSubscriber, IPXRowPersistedSubscriber
	{
		#region State
		...
		private static object SelectParent(PXCache cache, object row, object newRow, Type ParentType, PXParentAttribute attr)
		{
			...
			bool useCurrent = attr._UseCurrent;

			Type gType = cache.Graph.GetType();
			while (!useCurrent && gType.BaseType != null && gType.BaseType.IsGenericType)
			{
				Type[] gTypeArgs = gType.BaseType.GetGenericArguments();
				useCurrent = (gTypeArgs.Length > 1 && (gTypeArgs[1] == ParentType || ParentType.IsAssignableFrom(gTypeArgs[1])));
	...
	public class PXIntListAttribute : PXEventSubscriberAttribute, IPXFieldSelectingSubscriber, IPXLocalizableList
	{
			...
			: this()
		{
			...
			if (_AllowedValues.Length != allowedLabels.Length)
				throw new PXArgumentException("allowedLabels", ErrorMessages.IncorrectLabelArrayLength);
			_AllowedLabels = allowedLabels;
		...
		public virtual void FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			if (_AttributeLevel == PXAttributeLevel.Item || e.IsAltered)
			{
				...
				e.ReturnState = PXIntState.CreateInstance(e.ReturnState, _FieldName, null, null, null, null, _AllowedValues, _AllowedLabels, null, null, _NeutralAllowedLabels);
					}
				}
		#endregion
	}
	...
	internal sealed class PXDisplaySelectorAttribute : PXSelectorAttribute
	{
		...
		public override void SubstituteKeyFieldUpdating(PXCache sender, PXFieldUpdatingEventArgs e)
		{
			...
			if (!e.Cancel && e.NewValue != null)
			{
				...
				if (_CacheGlobal)
				{
					dict = PXContext.GetSlot<GlobalDictionary>(_Type.FullName);
					if (dict == null)
					{
						PXContext.SetSlot<GlobalDictionary>(_Type.FullName, (dict = PXDatabase.GetSlot<GlobalDictionary>(_Type.FullName, _BqlType)));
					}
					lock ((dict).SyncRoot)
					{
						KeyValuePair<object, bool> pair;
	...
	public class PXRestrictorAttribute : PXEventSubscriberAttribute, IPXFieldVerifyingSubscriber
	{
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			foreach (PXSelectorAttribute selattr in sender.GetAttributesReadonly(_FieldName).Where(attr => attr is PXSelectorAttribute).Cast<PXSelectorAttribute>())
			{
				...
				_Type = selattr.Type;
				if (_OriginalCmd.GetTables().Length < 2)
				{
		...
		public static object GetItem(PXCache cache, PXRestrictorAttribute attr, object data, object key)
		{
			...
			{
				dict = PXContext.GetSlot<PXSelectorAttribute.GlobalDictionary>(attr._Type.FullName);
				if (dict == null)
				{
					PXContext.SetSlot<PXSelectorAttribute.GlobalDictionary>(attr._Type.FullName, (dict = PXDatabase.GetSlot<PXSelectorAttribute.GlobalDictionary>(attr._Type.FullName, cache.Graph.Caches[attr._Type].BqlTable)));
				}
				lock ((dict).SyncRoot)
				{
					KeyValuePair<object, bool> pair;
		...
		public virtual void FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if (item != null)
			{
				...
				if (!_AlteredCmd.Meet(view.Cache, item, pars))
				{
					...
					throw new PXSetPropertyException(_Message, _MsgParams.Select(param => 
						sender.Graph.Caches[BqlCommand.GetItemType(param)].GetStateExt(PXResult.Unwrap(itemres, BqlCommand.GetItemType(param)), param.Name)).ToArray());
				}
			}
		}
	}
		...
		public PXZipValidationAttribute(Type zipValidationField, Type zipMaskField, Type countryIdField = null)
		{
			...
			if (countryIdField != null)
			{
				if (!typeof(IBqlField).IsAssignableFrom(countryIdField))
					throw new PXArgumentException("countryIdField", ErrorMessages.CountryIdFieldNotValid);
		...
		public override void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
			if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Select && (!sender.BypassCalced || sender.BqlSelect != null))
			{
				...
				(e.Operation & PXDBOperation.Option) == PXDBOperation.Internal))
				{
					...
					{
						string fieldtext = BqlCommand.GetSingleField(field, sender.Graph, new List<Type>(), null, BqlCommand.FieldPlace.Select);
						e.FieldName = BqlCommand.SubSelect + function + "(" + (String.IsNullOrEmpty(fieldtext) ? field.Name : fieldtext) + ") " + text + ")";
					}
	...
	public class PXUniqueAttribute : PXEventSubscriberAttribute, IPXRowPersistingSubscriber
	{
		...
		public override void CacheAttached(PXCache sender)
			...
			if (!typeof(IBqlField).IsAssignableFrom(_fieldType))
                throw new PXException(String.Format("The type {0} doesn't implement the IBqlField interface.", typeName));
			...
			if (WhereCondition != null)
				...
				if (!typeof(WhereBase<,,>).GenericIsAssignableFrom(WhereCondition))
					throw new PXException(String.Format("The type {0} is not a BQL Where condition.", WhereCondition.FullName));
	...
	public class PXGenerateAfterAttribute : Attribute
	{
		...
		{
			if (string.IsNullOrEmpty(fieldName)) throw new PXArgumentException(fieldName, "PXGenerateAfterAttribute fieldName must be set.");
			...
			foreach (string fieldName in cache.Fields)
			{
				...
				if (string.IsNullOrEmpty(followField)) result.Add(fieldName);
					...
					if (cache.Fields.Contains(followField))
						...
						throw new PXArgumentException(fieldName, "Invalid PXGenerateAfterAttribute parameter: No such DAC field has been found.");
				}
			}

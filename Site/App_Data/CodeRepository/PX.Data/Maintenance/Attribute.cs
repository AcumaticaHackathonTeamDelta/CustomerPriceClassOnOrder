...
namespace PX.Data
{
    ...
    /// <summary>The base class for <see cref="PXCacheNameAttribute">PXCacheName</see>
    /// and <see cref="PXViewNameAttribute">PXViewName</see> attributes. Do not use
    ...
    /// <see cref="PXUIFieldAttribute.GetItemName(PXCache)">GetItemName(PXCache)</see>
	...
	public class PXImportAttribute : PXViewExtensionAttribute
    {
			...
			where TBatchTable : class, IBqlTable, new()
		{
			...
			private void AskColumnAssociations()
			{
				...
				if (askIsOk)
				{
					...
					if (content != null && content.Length > 0)
					{
                        ...
                        foreach (Type t in _cache.Graph.Views.RestorableCaches.ToArray())
                        {
                            PXCache c = _cache.Graph.Caches[t];
                        }

                        foreach (var src in _cache.Graph.Caches.Caches)
                        {
                            var dest = importGraph.Caches[src.GetItemType()];

                            foreach (var row in src.Cached)
                            {
                                var status = src.GetStatus(row);
                                dest.SetStatus(row, status);
                                if (object.ReferenceEquals(row, src._Current))
                                {
                                    dest._Current = row;
                                }
                                if ((status == PXEntryStatus.Updated || status == PXEntryStatus.Inserted)
                                    && src.HasAttributes(row))
                                {
                                    foreach (PXEventSubscriberAttribute attr in src.GetAttributes(row, null))
                                    {
                                        if (attr is IPXInterfaceField)
                                        {
                                            IPXInterfaceField ui = (IPXInterfaceField)attr;
                                            if (ui.ErrorLevel == PXErrorLevel.Error && ui.ErrorValue != null)
                                            {
                                                PXUIFieldAttribute.SetError(dest, row, attr.FieldName, ui.ErrorText, ui.ErrorValue.ToString());
                                            }
                                        }
                                    }
                                }
						...
						PXLongOperation.StartOperation(_cache.Graph,
                            () =>
                            {
                                ...
                                _cache.Graph._UpdatePrimaryView(commonSettings.ViewName);
                                Thread.CurrentThread.CurrentCulture = threadCul;
                                _cache.Graph.Culture = graphCul;
			...
			{
				var sb = importGraph.GetType().
										GetField(this._viewName).
											GetValue(importGraph) as PXSelectBase;
				if (sb != null)
				{
					var importAttr = sb.GetAttribute<PXImportAttribute>();
					var importer = (PXImporter<TBatchTable>)importAttr._importer;
					...
					(sender, args) =>
					{
						if (!TryUploadData(_importer, importContentBag))
							throw new PXDialogRequiredException(_itemsViewName, null, "Validation failed",
																"The file, which is being uploaded, contains incorrect data.",

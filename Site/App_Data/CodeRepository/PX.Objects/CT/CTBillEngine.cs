...
namespace PX.Objects.CT
{
	public class CTBillEngine : PXGraph<CTBillEngine>
	{
		...
		public virtual void Setup(int? contractID, DateTime? date)
		{
			...
			foreach (PXResult<ContractItem, ContractDetail> res in PXSelectJoin<ContractItem, 
				InnerJoin<ContractDetail, On<ContractItem.contractItemID, Equal<ContractDetail.contractItemID>>>, 
				Where<ContractDetail.contractID, Equal<Required<Contract.contractID>>, 
					And<ContractDetail.revID, Equal<Required<Contract.revID>>>>>.Select(this, contract.ContractID, contract.RevID))
			{
		...
		public virtual void Activate(int? contractID, DateTime? date, bool setActivationDate=true)
		{
			...
			foreach (PXResult<ContractItem, ContractDetail> res in PXSelectJoin<ContractItem, InnerJoin<ContractDetail, On<ContractItem.contractItemID, Equal<ContractDetail.contractItemID>>>, Where<ContractDetail.contractID, Equal<Required<Contract.contractID>>, And<ContractDetail.revID, Equal<Required<Contract.revID>>>>>.Select(this, contract.ContractID, contract.RevID))
			{
				ContractItem item = res;
				ContractDetail detail = res;
				string message;
			...
			foreach (InvoiceData invData in invoices)
			{
				...
				switch (action)
				{
					...
					case ContractAction.Terminate:
						invoice.DocDesc = string.Format(PXMessages.LocalizeNoPrefix(Messages.TerminatingContract), contract.ContractCD, contract.Description);
						break;
					default:
		...
		private void Unlink(IEnumerable<ARInvoice> documents)
		{
			foreach (PMTran pMRef in documents
				.SelectMany(d => PXSelect<PMTran, Where<PMTran.projectID, Equal<Current<Contract.contractID>>,
					And<PMTran.aRTranType, Equal<Current<ARInvoice.docType>>,
					And<PMTran.aRRefNbr, Equal<Current<ARInvoice.refNbr>>>>>>.SelectMultiBound(this, new object[] {d})
					.RowCast<PMTran>()
		...
		{
			contract = PXCache<Contract>.CreateCopy(contract);
		...
		protected virtual List<UsageData> GetSetupFee(Contract contract)
		{
			...
			Where<ContractDetail.contractID, Equal<Required<Contract.contractID>>,
				And<ContractDetail.isBaseValid, Equal<True>>>>.Select(this, contract.ContractID))
			{
				...
				data.Description = string.Format(PXMessages.LocalizeNoPrefix(Messages.SetupDescription), (string)this.Caches[typeof(InventoryItem)].GetValueExt<InventoryItem.descr>(inventory));
		...
		protected virtual List<UsageData> GetActivationFee(Contract contract, DateTime? date)
		{
			...
			foreach (PXResult<ContractDetail, ContractItem, InventoryItem> res in renewalItemsSelect.Select(contract.ContractID))
			{
				...
				if (item.CollectRenewFeeOnActivation == true)
				{
					...
					//it's ok to use current culture here as this method is always executing in the locale scope
					data.Description = string.Format(PXMessages.LocalizeNoPrefix(Messages.ActivateRenew), (string)this.Caches[typeof(InventoryItem)].GetValueExt<InventoryItem.descr>(inventory));
					data.UOM = inventory.BaseUnit;
					data.Qty = det.Qty;
		...
		protected virtual void GetUpgradeSetup(IEnumerable<PXResult<ContractDetail, ContractItem>> details, List<UsageData> usagedata, decimal? prorate)
		{
			foreach (PXResult<ContractDetail, ContractItem> res in details)
			{
				...
				if (item.BaseItemID != null && (change != 0 && (change > 0 || item.Refundable == true)))
				{
					...
					data.InventoryID = item.BaseItemID;
					//it's ok to use current culture here as this method is always executing in the locale scope
					data.Description = string.Format(PXMessages.LocalizeNoPrefix(Messages.SetupUpgrade), (string)this.Caches[typeof(InventoryItem)].GetValueExt<InventoryItem.descr>(inventory));
					...
					if (item.RenewalItemID != null && item.CollectRenewFeeOnActivation == true)
					{
						...
						//it's ok to use current culture here as this method is always executing in the locale scope
						renew.Description = string.Format(PXMessages.LocalizeNoPrefix(Messages.UpgradeActivation), (string)this.Caches[typeof(InventoryItem)].GetValueExt<InventoryItem.descr>(inventory));
		...
		protected virtual void GetUpgradeRecurring(IEnumerable<PXResult<ContractDetail, ContractItem>> details, List<UsageData> usagedata, decimal? prorate)
		{
			foreach (PXResult<ContractDetail, ContractItem> res in details)
			{
				...
				if (item.RecurringItemID != null && item.DepositItemID == null && (item.RecurringType == RecurringOption.Prepay || change < 0m))
				{
					...
					//it's ok to use current culture here as this method is always executing in the locale scope
					data.Description = (string)this.Caches[typeof(InventoryItem)].GetValueExt<InventoryItem.descr>(inventory);
					data.UOM = inventory.BaseUnit;
			...
			foreach (PXResult<ContractDetail, ContractItem> res in details)
			{
				...
				if (item.RecurringItemID != null && item.DepositItemID == null)
				{
					...
					if (unusedQty > 0)
					{
						...
						//it's ok to use current culture here as this method is always executing in the locale scope
						unusedData.Description = (string)this.Caches[typeof(InventoryItem)].GetValueExt<InventoryItem.descr>(inventory);
					...
					if (usedQty > 0 && item.RecurringType != RecurringOption.Prepay)
					{
						...
						//it's ok to use current culture here as this method is always executing in the locale scope
						usedData.Description = (string)this.Caches[typeof(InventoryItem)].GetValueExt<InventoryItem.descr>(inventory);
		...
		protected virtual void GetUpgradeUsage(Contract contract, IEnumerable<PXResult<ContractDetail, ContractItem>> details, List<UsageData> list, out Dictionary<int, List<TranNotePair>> sourceTran, out List<UsageData> tranData)
		{
			...
			foreach (KeyValuePair<int, List<TranNotePair>> kv in sourceTran)
				...
				PXResult<ContractDetail, ContractItem, InventoryItem> billing = (details
					...
					.Select(@t => new {@t, inventory = PXSelect<InventoryItem, Where<InventoryItem.inventoryID, Equal<Required<InventoryItem.inventoryID>>>>.Select(this, ((ContractItem) @t.item).RecurringItemID)})
					.Select(@t => new PXResult<ContractDetail, ContractItem, InventoryItem>(@t.@t.res, @t.@t.res, @t.inventory))).FirstOrDefault();
		...
		protected virtual List<UsageData> GetRenewalUsage(Contract contract)
		{
			...
			PXSelectBase<ContractDetail> renewalItemsSelect = new
					 ...
					 Where<ContractDetail.contractID, Equal<Required<Contract.contractID>>,
					 And<ContractDetail.isRenewalValid, Equal<True>>>>(this);
			Customer customer = PXSelectReadonly<Customer, Where<Customer.bAccountID, Equal<Required<Customer.bAccountID>>>>.Select(this, contract.CustomerID);
			foreach (PXResult<ContractDetail, ContractItem, InventoryItem> res in renewalItemsSelect.Select(contract.ContractID))
			{
				ContractDetail det = res;
				ContractItem item = res;
				...
				UsageData data = new UsageData();
				data.InventoryID = inventory.InventoryID;
				data.Description = PXDBLocalizableStringAttribute.GetTranslation(Caches[typeof(InventoryItem)], inventory, nameof(InventoryItem.Descr), customer?.LocaleName);
				data.UOM = inventory.BaseUnit;
				data.Qty = det.Qty;
				data.PriceOverride = det.RenewalPriceVal;
				data.ExtPrice = det.Qty * det.RenewalPriceVal;
				...
				foreach (PXResult<ContractDetail, ContractItem> res in details)
				{
					...
					if (item.RecurringItemID != null && item.DepositItemID == null)
					{
						...
						UsageData data = new UsageData
						{
							...
							Description = PXDBLocalizableStringAttribute.GetTranslation(Caches[typeof(InventoryItem)], inventory, nameof(InventoryItem.Descr), customer?.LocaleName),
		...
		protected virtual void SetBillingTarget(Contract contract, out Customer customer, out Location location)
		{
			...
			if (contract.CustomerID != null)
			{
				...
				location = locationID != null
					? PXSelect<Location, Where<Location.bAccountID, Equal<Required<ContractBillingSchedule.accountID>>, And<Location.locationID, Equal<Required<ContractBillingSchedule.locationID>>>>>.Select(this, customer.BAccountID, locationID)
					: PXSelect<Location, Where<Location.bAccountID, Equal<Required<Customer.bAccountID>>, And<Location.locationID, Equal<Required<Customer.defLocationID>>>>>.Select(this, customer.DefLocationID);
			}
		}
		...
		protected virtual Dictionary<int, List<TranNotePair>> GetTransactions(Contract contract)
		{
			...
			else
			{
				...
				resultset = PXSelectJoin<PMTran,
					   ...
					   And<PMTran.date, Less<Required<PMTran.date>>>>>>.Select(this, contract.ContractID, includeDate);
			}
		...
		protected virtual List<UsageData> ProcessTransactions(Contract contract, Dictionary<int, List<TranNotePair>> transactions)
		{
			...
			foreach (KeyValuePair<int, List<TranNotePair>> kv in transactions)
			{
				...
				{
					billing = (PXResult<ContractDetail, ContractItem, InventoryItem>) PXSelectJoin<ContractDetail,
					InnerJoin<ContractItem, On<ContractItem.contractItemID, Equal<ContractDetail.contractItemID>>,
					InnerJoin<InventoryItem, On<InventoryItem.inventoryID, Equal<ContractItem.recurringItemID>>>>,
					Where<ContractDetail.contractID, Equal<Required<Contract.contractID>>,
				...
				if (billing == null)
				{
					InventoryItem inventoryItem = PXSelect<InventoryItem, Where<InventoryItem.inventoryID, Equal<Required<InventoryItem.inventoryID>>>>.Select(this, kv.Key);
		...
		protected virtual void UpdateReferencePMTran2ARTran(ARInvoiceEntry graph, Dictionary<int, List<TranNotePair>> sourceTran, List<UsageData> tranData)
		{
			...
			foreach (UsageData ud in tranData)
			{
				foreach (PMTran source in ud.TranIDs.Cast<long>().Select(tranID => sourceTranDict[tranID]))
				{
					...
					graph.RefPMTran.Update(source); //Note: PMTran should be saved via InvoiceEntry so that reference is updated correctly.
				}
			}
		}
		...
		protected virtual void RaiseErrorIfUnreleasedUsageExist(Contract contract)
		{
			...
			PXSelectBase<CRPMTimeActivity> selectActivities = new 
				PXSelectJoin<
					CRPMTimeActivity,
				InnerJoin<CRCase, 
			...
			{
				failed = true;
				sb.AppendFormat("Case: {0} Activity: {1}{2}", ((CRCase)res).CaseCD, ((CRPMTimeActivity)res).Subject, Environment.NewLine);
			}

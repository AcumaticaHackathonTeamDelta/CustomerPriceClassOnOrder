...
namespace PX.Objects.CT
{
		...
		where PriceDate : IBqlOperand
	{
		public override object Evaluate(PXCache cache, object item, Dictionary<Type, object> pars)
		{
			...
			//to do: modify this part
			PXResult<Contract, ContractBillingSchedule> customerContract = (PXResult<Contract, ContractBillingSchedule>)PXSelectJoin<Contract, LeftJoin<ContractBillingSchedule, On<ContractBillingSchedule.contractID, Equal<Contract.contractID>>>, Where<Contract.contractID, Equal<Required<Contract.contractID>>>>.Select(cache.Graph, contractID);
		...
		public virtual decimal GetItemPrice(PXCache sender, string curyID, int? customerID, int? locationID, string contractStatus, int? contractItemID, int? itemID, string itemType, string priceOption, decimal? fixedPrice, decimal? setupPrice, decimal? qty, DateTime? date)
		{
			...
			if (item != null)
			{
				...
				string customerPriceClass = AR.ARPriceClass.EmptyPriceClass;
				CR.Location c = PXSelect<CR.Location, Where<CR.Location.bAccountID, Equal<Required<Contract.customerID>>, And<CR.Location.locationID, Equal<Required<Contract.locationID>>>>>.Select(sender.Graph, customerID, locationID); // Location.Select();

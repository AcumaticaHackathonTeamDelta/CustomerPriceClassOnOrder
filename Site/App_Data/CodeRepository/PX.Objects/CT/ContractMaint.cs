...
namespace PX.Objects.CT
{
	public class ContractMaint : PXGraph<ContractMaint, Contract>
	{
	   ...
	   [PXCopyPasteHiddenView]
		...
		{
			if (RenewalSettings.AskExt() == WebDialogResult.OK && RenewalSettings.VerifyRequired() && RenewalSettings.Current.RenewalDate != null)
			{
			...
			if (contract.Type == Contract.type.Expiring || (contract.Type == Contract.type.Renewable && IsExpired(contract, renewalDate)))
		{
				...
				if (!isAutonumbered && renewManualNumberingFilter.AskExt() == WebDialogResult.OK)
		{
		...
		public virtual void Bill()
		{
			...
			DateTime? billingDate = Billing.Current.Type == BillingType.OnDemand && OnDemandSettings.AskExt() == WebDialogResult.OK ? OnDemandSettings.Current.BillingDate : null;
			PXLongOperation.StartOperation(this, delegate
			{
				CTBillEngine engine = CreateInstance<CTBillEngine>();
		...
		public virtual IEnumerable Setup(PXAdapter adapter)
		{
			...
			if (SetupSettings.AskExt((graph, viewName) =>
			{
				SetupSettings.Cache.SetDefaultExt<SetupSettingsFilter.startDate>(SetupSettings.Current);
			}) == WebDialogResult.OK)
			{
		...
		protected virtual void SetControlsState(Contract row, PXCache cache)
		{
			...
			PXUIFieldAttribute.SetEnabled<Contract.caseItemID>(cache, row, !row.TemplateID.HasValue || (!cancelled && template != null && template.AllowOverride == true));

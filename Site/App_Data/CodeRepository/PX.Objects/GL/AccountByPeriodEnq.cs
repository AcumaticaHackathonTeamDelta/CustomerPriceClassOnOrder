...
namespace PX.Objects.GL
{
	...
	public partial class AccountByPeriodFilter : IBqlTable
	{
		...
		[PXDate()]
		[PXUIField(DisplayName = "Period End Date", Visibility = PXUIVisibility.Visible, Visible = true, Enabled = false)]
		public virtual DateTime? PeriodEndDateUI => _PeriodEndDate?.AddDays(-1) ?? _PeriodEndDate;
	...
	public class AccountByPeriodEnq : PXGraph<AccountByPeriodEnq>
	{
		...
		public virtual IEnumerable viewDocument(PXAdapter adapter)
		{
			...
			if (tran != null)
			{
				PXGraph graph = new GraphFactory()[tran.Module]?.Create(tran);
				throw graph != null 
					? new PXRedirectRequiredException(graph, true, Messages.ViewSourceDocument) { Mode = PXBaseRedirectException.WindowMode.NewWindow} 
					: new PXException(Messages.SourceDocumentCanNotBeFound);
			}
			return Filter.Select();
		}
		...
		protected virtual IEnumerable glTranEnq()
		{
			...
			return Command.View.Select(null, null, PXView.Searches, PXView.SortColumns, PXView.Descendings, TransactionFilters, ref startRow, isSyncPosition ? PXView.MaximumRows : 0, ref totalRows)
		...
		protected virtual void GLTranR_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			bool tranSelectableForReclass = !JournalEntry.IsTransactionReclassifiable(tran, tran.BatchType, CurrentLedger.BalanceType, ProjectDefaultAttribute.NonProject(this)) 
		...
		public virtual PXGraph Create(string aTranType, string aRefNbr, int? referenceID)
		{
			switch(aTranType)
			{
				...
				case CATranType.CAAdjustment:
				{
					...
					destGraph.CAAdjRecords.Current = destGraph.CAAdjRecords.Search<CAAdj.adjRefNbr>(aRefNbr, aTranType);
					return destGraph;
				}

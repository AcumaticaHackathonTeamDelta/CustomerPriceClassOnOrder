...
namespace PX.Objects.CR
{
	public class CREmailActivityMaint : CRBaseActivityMaint<CREmailActivityMaint, CRSMEmail>
	{
		...
		public virtual void loadEmailSource()
		{
			...
			if (res == WebDialogResult.OK)
			{
				if (NotificationInfo.Current.Type == TemplateSourceType.Notification)
				{
					...
					PXPrimaryGraphAttribute.FindPrimaryGraph(this.Caches[type], ref row, out graphType);
					...
					PXNoteAttribute.SetFileNotes(Activities.Cache, Activities.Current, _attachments.ToArray());

					Activities.Current.MailAccountID = upd.NFrom ?? Activities.Current.MailAccountID;

					if (NotificationInfo.Current.AppendText == true)
					{
						Activities.Current.MailTo = AppendMainAddress(Activities.Current.MailTo, upd.NTo);
						Activities.Current.MailCc = AppendMainAddress(Activities.Current.MailCc, upd.NCc);
						Activities.Current.MailBcc = AppendMainAddress(Activities.Current.MailCc, upd.NBcc);
						Activities.Current.Subject += upd.Subject;
                        Activities.Current.Body = Activities.Current.Body + upd.Body;
					}
					else
					{
						Activities.Current.MailTo = upd.NTo;
						Activities.Current.MailCc = upd.NCc;
						Activities.Current.MailBcc = upd.NBcc;
						Activities.Current.Body = upd.Body;
						Activities.Current.Subject = upd.Subject;
					}
				}
				if (NotificationInfo.Current.Type == TemplateSourceType.Activity)
				{
					...
					if (NotificationInfo.Current.AppendText == true)
					{
						Activities.Current.Body = Activities.Current.Body + email.Body;
					}
					else
					{
						Activities.Current.Body = email.Body;
					}
				}
				if (NotificationInfo.Current.Type == TemplateSourceType.KnowledgeBase)
				{
					...
					if (NotificationInfo.Current.AppendText == true)
					{
						Activities.Current.Body = Activities.Current.Body + PXWikiParser.Parse(wr.Content);
					}
					else
					{
						Activities.Current.Body = PXWikiParser.Parse(wr.Content);
					}
				}
			}
            Activities.Current.Body = PX.Web.UI.PXRichTextConverter.NormalizeHtml(Activities.Current.Body);
		}
		...
		public override void Persist()
		{
			...
			this.Activities.Cache.RaiseRowSelected(this.Activities.Cache.Current);			
		}
		...
		protected virtual void CRSMEmail_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			string origStatus =
				(string)this.Activities.Cache.GetValueOriginal<CRSMEmail.uistatus>(row) ?? ActivityStatusListAttribute.Open;
			...
			PXDefaultAttribute.SetPersistingCheck<CRSMEmail.ownerID>(cache, row, !this.UnattendedMode && tAct?.TrackTime == true && row.MPStatus != MailStatusListAttribute.Deleted ? PXPersistingCheck.Null : PXPersistingCheck.Nothing);
		...
		private static string FillMailReply(PXGraph graph, CRSMEmail message)
		{
			...
			if (message.MailAccountID != null)
			{
				...
				Mailbox.TryParse(message.MailFrom, out mailReply);
		...
		private static string FillMailFrom(PXGraph graph, CRSMEmail message, bool allowUseCurrentUser = false)
			...
			if (message.MailAccountID != null)
			{
				...
				Mailbox.TryParse(message.MailFrom, out mailFrom);
		...
		private static string GenerateBackAddress(PXGraph graph, CRSMEmail message, string defaultDisplayName, string defaultAddress, bool allowUseCurrentUser)
		{
				...
				if (message.OwnerID == null || message.ClassID == CRActivityClass.EmailRouting)
			{
				...
				result = string.IsNullOrEmpty(displayName)
					? address
					: Mailbox.Create(displayName, address);
			}
			...
			if (string.IsNullOrEmpty(result) && allowUseCurrentUser)
			{
				return graph.Accessinfo.UserID.
					...
					With(u => Mailbox.Create(u.FullName, u.Email));
			}
		...
		private static string AppendMainAddress(string address1, string address2)
		{
			if (string.IsNullOrEmpty(address1)) return address2;
			if (string.IsNullOrEmpty(address2)) return address1;
		...
		private void TryCorrectMailDisplayNames(CRSMEmail message)
		{
			...
			Mailbox fromBox;
			if (ownerEmail != null && Mailbox.TryParse(ownerEmail, out fromBox))
		...
		private static int IndexOf(string tag, string message)
		{
			if (message == null) return -1;
			var index = message.IndexOf("<" + tag + ">");
			return (index == -1) ?
				 message.IndexOf("<" + tag + " ") : index;
		}

		private void CorrectFileNames()
		{
			var noteId = Message.Current.With(m => m.NoteID);
			var actNoteId = Activities.Current.With(act => act.NoteID);
			if (noteId == null || actNoteId == null) return;

			var searchText = "[" + Message.Current.MessageId + "]";
			var replaceText = "[" + Activities.Current.NoteID + "]";

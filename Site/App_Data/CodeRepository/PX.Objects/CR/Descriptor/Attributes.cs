...
namespace PX.Objects.CR
{
	...
	public class StateAttribute : PXAggregateAttribute, IPXFieldVerifyingSubscriber
	{
		...
		public class StateSelectorAttribute : PXSelectorAttribute
		{
			public StateSelectorAttribute(Type SearchType):
				base(SearchType)
			{
		...
		public StateAttribute(Type aCountryID)
		{
			...
			_Attributes.Add(new StateSelectorAttribute(SearchType));
	...
	{		
		...
		public override void FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			var addr = e.NewValue == null
				? new AddressList()
				: AddressList.Parse(e.NewValue.ToString());

			foreach (Mailbox mailbox in addr.Mailboxes)
			{
				if (!IsValid(mailbox.Address))
				{
					throw new PXSetPropertyException("Invalid email");
				}
			}
		}
	...
	public class CROpportunityStagesAttribute : PXStringListAttribute
	{
		...
		public sealed class Prospect : Constant<string>
		{
			public Prospect()
				: base(_PROSPECT)
			{

...
namespace PX.Objects.CR
{
                                    ...
                                    And<CRSMEmail.mpstatus, NotEqual<MailStatusListAttribute.archived>>>>>
	{
		...
		public virtual IEnumerable relate(PXAdapter adapter)
		{
			...
			if (lint)
			{
				...
				foreach (PXResult<CRSMEmail, EMailAccount, EPView> email in SelectedList())
				{
				    ...
				    object row = helper.GetEntityRow(entityType, entityFilter.Current.NoteID);
					...
					copy.Exception = null;
				    PXRefNoteSelectorAttribute.EnsureNotePersistence(this, entityFilter.Current.Type, entityFilter.Current.NoteID);
		...
		protected virtual IEnumerable MarkAs(PXAdapter adapter, int vStatus, IEnumerable<PXResult<CRSMEmail, EMailAccount, EPView>> messages)
		{            
            foreach (PXResult<CRSMEmail, EMailAccount, EPView> r in messages)
			{
				...
				if ((owner == null || email.OwnerID != Accessinfo.UserID) && vstatus.Status != vStatus)
				{
					Emails.Cache.RaiseExceptionHandling<CRSMEmail.selected>(email, null, new PXSetPropertyException(Messages.OnlyOwnerSetReadStatus, PXErrorLevel.RowWarning, new EPViewStatusAttribute().ValueLabelDic[vStatus]));
				}
				else if (vStatus == EPViewStatusAttribute.VIEWED
        ...
        static void ArchiveMessages(IEnumerable<CRSMEmail> messages)
        {
            CREmailActivityMaint graph = CreateInstance<CREmailActivityMaint>();

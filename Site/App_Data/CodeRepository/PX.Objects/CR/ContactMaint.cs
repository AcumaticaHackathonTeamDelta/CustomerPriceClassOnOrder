...
namespace PX.Objects.CR
{
	public class ContactMaint : PXGraph<ContactMaint, Contact, Contact.displayName>
	{
		...
		public PXSelectJoin<Contact, LeftJoin<CRActivityStatistics, On<Contact.noteID, Equal<CRActivityStatistics.noteID>>>,
		...
		[PXFilterable]
		[CRDefaultMailTo]
		[CRReference(typeof(Select<BAccountR, Where<BAccountR.bAccountID, Equal<Current<Contact.bAccountID>>>>),
			typeof(Select<Contact, Where<Contact.contactID, Equal<Current<Contact.contactID>>>>))]
		...
		public virtual IEnumerable ConvertToBAccount(PXAdapter adapter)
		{
			...
			foreach (Contact contact in contacts.Where(contact => contact != null))
			{
				...
				Save.Press();
				Contact c = contact;
				PXLongOperation.StartOperation(this, () => LeadMaint.ConvertToAccount(c, AccountInfo.Current));
			}
			return contacts;
		}	
		#endregion

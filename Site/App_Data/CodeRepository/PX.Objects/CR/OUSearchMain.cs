    ...
    <GetItem
      ...
      <ItemIds>
		...
		protected virtual void OUCase_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			{
				List<object> contracts = PXSelectorAttribute.SelectAll<OUCase.contractID>(sender, e.Row);
				if (contracts.Count == 1)
				{
					NewCase.SetValueExt<OUCase.contractID>(row, PXResult.Unwrap<Contract>(contracts[0]).ContractID);
				}
			}
		}
		...
		{		
			...
			externalLink +=  PX.Common.PXUrl.ToAbsoluteUrl(popup ? node.Url : "~/Main.aspx");			
		...
		{			
            ...
            if (message == null)
			{
				...
				{
					var attachments = new List<PX.SM.FileInfo>();
					foreach (var attachDetail in exchangeMessage.Attachments)
				{
						var fileInfo = this.GetAttachmentsFromExchangeServerUsingEWS(this.SourceMessage.Current.EwsUrl,
							this.SourceMessage.Current.Token, attachDetail);
					...
					PXNoteAttribute.SetFileNotes(this.Caches[typeof(CRSMEmail)], 
						msg, attachments.Where(x => x.UID != null).Select(x => x.UID.Value).ToArray());
				}
			}
			else
				msg = message;
			...
			msg.Subject = this.SourceMessage.Current.Subject == String.Empty ? noSubject : this.SourceMessage.Current.Subject;
			msg.RefNoteID = refNoteID;
			msg.BAccountID = bAccountID;
		    msg.ContactID = contactID;
            graph.Activities.Update(msg);
			graph.Save.PressImpl(false, true);
		...
		private void UploadAttachments(List<PX.SM.FileInfo> files)
		{
		...
		protected virtual void OUSearchEntity_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
        {
            ...
            ViewContact.SetVisible(contact != null && row.Operation == null && IsRights(typeof(ContactMaint), typeof(Contact), PXCacheRights.Select));
            ViewBAccount.SetVisible(contact != null && row.Operation == null && contact.BAccountID != null && IsRights(typeof(BusinessAccountMaint), typeof(BAccount), PXCacheRights.Select));
		...
		private PX.SM.FileInfo GetAttachmentsFromExchangeServerUsingEWS(string ewsUrl, string token, AttachmentDetails detail)
        {
			if (detail == null)
				return null;
		...
		private List<PX.SM.FileInfo> ProcessAttachmentXmlResponse(XElement responseEnvelope)
        {
            var result = new List<PX.SM.FileInfo>();
            ...
            foreach (var fileAttachment in fileAttachments)
            {
                ...
                if (fileData != null)
                {
                    ...
                    PX.SM.FileInfo file = new PX.SM.FileInfo(fileName.Value, null, fileData);
                    upload.SaveFile(file, PX.SM.FileExistsAction.CreateVersion);
                    result.Add(file);
                }
            }
		...
		private XElement SendRequestToExchange(string url, string token, string soapRequest)
		{
			try
			{
				...
				using (HttpWebResponse webResponse = (HttpWebResponse)webRequest.GetResponse())
				{
					...
					{
						this.Filter.Current.ErrorMessage = "Error occured: " + webResponse.StatusDescription;
						return null;
					}
				}
			}
				...
				using (WebResponse response = ex.Response)
				{
					...
					using (Stream data = response.GetResponseStream())
					{
						using (StreamReader reader = new StreamReader(data))
						{
							...
							throw PXException.PreserveStack(new PXException("The system cannot execute a web request." + Environment.NewLine + text));
						}
					}
				}
			}

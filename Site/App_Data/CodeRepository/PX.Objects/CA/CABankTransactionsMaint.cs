...
namespace PX.Objects.CA
{
	public class CABankTransactionsMaint : PXGraph<CABankTransactionsMaint>
	{
			...
			: base()
		{
			...
			matchSettingsPanel.StateSelectingEvents += new PXFieldSelecting(delegate(PXCache sender, PXFieldSelectingEventArgs e)
			{
				if (PXLongOperation.GetStatus(this.UID, out timespan, out message) != PXLongRunStatus.NotExists)
				{
					e.ReturnState = PXButtonState.CreateInstance(e.ReturnState, null, null, null, null, null, false,
						PXConfirmationType.Unspecified, null, null, null, null, null, null, null, null, null, null, null, typeof(Filter));
					((PXButtonState)e.ReturnState).Enabled = false;
				}
			});

			processMatched.StateSelectingEvents += new PXFieldSelecting(delegate(PXCache sender, PXFieldSelectingEventArgs e)
			{
				if (PXLongOperation.GetStatus(this.UID, out timespan, out message) != PXLongRunStatus.NotExists)
				{
					e.ReturnState = PXButtonState.CreateInstance(e.ReturnState, null, null, null, null, null, false,
			...
			uploadFile.StateSelectingEvents += new PXFieldSelecting(delegate(PXCache sender, PXFieldSelectingEventArgs e)
			{
				if (PXLongOperation.GetStatus(this.UID, out timespan, out message) != PXLongRunStatus.NotExists)
				{
					e.ReturnState = PXButtonState.CreateInstance(e.ReturnState, null, null, null, null, null, false,
			...
			autoMatch.StateSelectingEvents += new PXFieldSelecting(delegate(PXCache sender, PXFieldSelectingEventArgs e)
			{
				if (PXLongOperation.GetStatus(this.UID, out timespan, out message) != PXLongRunStatus.NotExists)
				{
					e.ReturnState = PXButtonState.CreateInstance(e.ReturnState, null, null, null, null, null, false,
		...
		public partial class Filter : IBqlTable
		{
			...
			[CashAccount(null, typeof(Search<CashAccount.cashAccountID, Where2<Match<Current<AccessInfo.userName>>, And<CashAccount.clearingAccount, Equal<CS.boolFalse>>>>), IsKey = true)]
		...
		public PXSelect<CABankTranAdjustment, Where<CABankTranAdjustment.tranID, Equal<Optional<CABankTran.tranID>>>> Adjustments;
		...
		[PXCopyPasteHiddenView]
		public PXSelect<Customer, Where<Customer.bAccountID, Equal<Current<CABankTran.payeeBAccountID>>>> currentCustomer;
		...
		public virtual IEnumerable LoadInvoices(PXAdapter adapter)
		{
			...
			if (currentDoc.OrigModule == GL.BatchModule.AR)
			{
				...
				PXResultset<ARInvoice> custdocs = ARPaymentEntry.GetCustDocs(null, payment, arsetup.Current, this);
		...
		protected IEnumerable Cancel(PXAdapter adapter)
		{
			...
			{
				foreach (var item in new PXCancel<Filter>(this, ActionsMessages.Cancel).Press(adapter))
				{
					yield return item;
				}
			}
		...
		protected virtual bool ApplyInvoiceInfo(PXCache sender, CABankTran row)
		{
			if (row.CreateDocument == true && row.InvoiceInfo != null)
			{
				...
				if (linkedInvoice != null)
				{
					...
					//sender.SetValueExt<CABankTran.payeeBAccountID>(row, payeeBAccountID);
		...
		protected virtual void CAAdj_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			var doc = e.Row as CAAdj;
			if (doc == null)
				return;

			PXDefaultAttribute.SetPersistingCheck<CAAdj.extRefNbr>(cache, doc, CASetup.Current.RequireExtRefNbr == true ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
		}

		protected virtual void CADeposit_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			var doc = e.Row as CADeposit;
			if (doc == null)
				return;

			PXDefaultAttribute.SetPersistingCheck<CADeposit.extRefNbr>(cache, doc, CASetup.Current.RequireExtRefNbr == true ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
		}
		...
		protected virtual void CABankTran_CreateDocument_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (row.CreateDocument == true)
			{
				...
				if (row.CreateDocument == true && isInvoiceFound == false)
				{
		...
		protected virtual void ResetTranFields(PXCache cache, CABankTran transaction)
		{
			...
			cache.SetDefaultExt<CABankTran.curyApplAmt>(transaction);
			cache.SetDefaultExt<CABankTran.curyUnappliedBal>(transaction);
			cache.SetDefaultExt<CABankTran.curyApplAmtCA>(transaction);
			cache.SetDefaultExt<CABankTran.curyUnappliedBalCA>(transaction);
		}
		...
		protected virtual void CABankTran_DocumentMatched_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (row.DocumentMatched == true)
			{
				...
				if (match != null && !string.IsNullOrEmpty(match.DocRefNbr))
				{
					...
					//Details.Cache.SetValueExt<CABankTran.payeeBAccountIDCopy>(row, match.ReferenceID);
				}
			}
		}
		...
		protected virtual void CABankTran_CashAccountID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			CABankTran adj = (CABankTran)e.Row;
			adj.Cleared = false;
			if (cashAccount.Current == null || cashAccount.Current.CashAccountID != adj.CashAccountID)
			{
				cashAccount.Current = (CashAccount)PXSelectorAttribute.Select<CABankTran.cashAccountID>(sender, adj);
			}
			...
			{
				if (e.ExternalCall || sender.GetValuePending<CABankTran.curyID>(adj) == null)
				{
					CurrencyInfo info = CurrencyInfoAttribute.SetDefaults<CABankTran.curyInfoID>(sender, adj);

					string message = PXUIFieldAttribute.GetError<CurrencyInfo.curyEffDate>(currencyinfo.Cache, info);
					if (string.IsNullOrEmpty(message) == false)
					{
						sender.RaiseExceptionHandling<CABankTran.tranDate>(adj, adj.TranDate, new PXSetPropertyException(message, PXErrorLevel.Warning));
					}
		...
		protected virtual void CABankTran_MatchStatsInfo_FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			...
			if (row != null)
			{
				...
				{
					//message = PXMessages.LocalizeFormatNoPrefix(Messages.TRansactionNotMatchedThereAreMatchCandididated, (row.CountMatches ?? 0), (row.CountInvoiceMatches ?? 0));
		...
		private static void RaiseOrHideError<T>(PXCache cache, object row, bool isIncorrect, string message, PXErrorLevel errorLevel, params object[] parameters)
			where T : IBqlField
		{
			if (isIncorrect)
			{
...
#if false
        protected virtual void CABankTran_DocumentMatched_FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
        {
            CABankTran row = (CABankTran)e.Row;
            if (row != null && row.DocumentMatched != true)
            {
                IEnumerable matchesTran = this.FindDetailMatches(row, this.matchSettings.Current, false);
                IEnumerable matchesInv = this.FindDetailMatchingInvoices(row, this.matchSettings.Current, false);
            }
        } 
#endif
		#endregion
		...
		protected virtual void CABankTranDetail_AccountID_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			if ((tran == null) || (tran.EntryTypeID == null) || (split == null)) return;
		...
		protected virtual void CABankTranDetail_AccountID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			...
			if (glAccount != null && glAccount.IsCashAccount == true)
			{
				...
				if (row.SubID != null && row.BranchID != null)
				{
					cashAcct = PXSelect<CashAccount, Where<CashAccount.accountID, Equal<Required<CABankTranDetail.accountID>>,
						  And<CashAccount.subID, Equal<Required<CABankTranDetail.subID>>,
						  And<CashAccount.branchID, Equal<Required<CABankTranDetail.branchID>>>>>>.Select(this, glAccount.AccountID, row.SubID, row.BranchID);
				}
				else if (row.BranchID != null)
				{
					cashAcct = PXSelect<CashAccount, Where<CashAccount.accountID, Equal<Required<CABankTranDetail.accountID>>,
						  And<CashAccount.branchID, Equal<Required<CABankTranDetail.branchID>>>>>.Select(this, glAccount.AccountID, row.BranchID);
				}
				else if (row.SubID != null)
				{
					cashAcct = PXSelect<CashAccount, Where<CashAccount.accountID, Equal<Required<CABankTranDetail.accountID>>,
						  And<CashAccount.subID, Equal<Required<CABankTranDetail.subID>>>>>.Select(this, glAccount.AccountID, row.SubID);
				}
				if (cashAcct == null)
				{
					cashAcct = PXSelect<CashAccount,
					Where<CashAccount.accountID, Equal<Required<CashAccount.accountID>>>>.Select(this, glAccount.AccountID);
				}
				if (cashAccount != null)
				{
				cache.SetValueExt<CABankTranDetail.branchID>(row, cashAcct.BranchID);
				cache.SetValueExt<CABankTranDetail.subID>(row, cashAcct.SubID);
			}
		}
		}
		...
		protected virtual void CABankTranDetail_CashAccountID_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			{
				CashAccount CashAccount = PXSelect<CashAccount, Where<CashAccount.accountID, Equal<Required<CABankTranDetail.accountID>>,
						And<CashAccount.subID, Equal<Required<CABankTranDetail.subID>>, 
						And<CashAccount.branchID, Equal<Required<CABankTranDetail.branchID>>>>>>.Select(sender.Graph, split.AccountID, split.SubID, split.BranchID);
				if (CashAccount != null)
				{
		...
		protected virtual void CABankTranDetail_CashAccountID_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if (CashAccount != null)
			{
				if (CashAccount.AccountID == Details.Current.CashAccountID)
				{
					e.NewValue = CashAccount.CashAccountCD;
		...
		protected virtual void CABankTranDetail_CashAccountID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (split.CashAccountID != (int?)e.OldValue)
			{
				if (split.CashAccountID != null)
				{
					...
					{
						sender.SetValue<CABankTranDetail.accountID>(split, offsetCashAcct.AccountID);
						sender.SetValue<CABankTranDetail.subID>(split, offsetCashAcct.SubID);
						sender.SetValue<CABankTranDetail.branchID>(split, offsetCashAcct.BranchID);
					}
				}
				else
				{
					sender.SetValue<CABankTranDetail.accountID>(split, null);
					sender.SetValue<CABankTranDetail.subID>(split, null);
					sender.SetValue<CABankTranDetail.branchID>(split, null);
				}
			}
		}
		...
		{
			CABankTranDetail oldSplit = e.Row as CABankTranDetail;
			CABankTranDetail newSplit = e.NewRow as CABankTranDetail;
			if (newSplit == null) return;
			updateAmountPrice(oldSplit, newSplit);

			if (newSplit.AccountID != null)
			{
				Account currentAcc = PXSelect<Account, Where<Account.accountID, Equal<Required<Account.accountID>>>>.Select(this, newSplit.AccountID);
				if (currentAcc.IsCashAccount == true)
				{
					CashAccount cashAccount = PXSelect<CashAccount, Where<CashAccount.accountID, Equal<Required<CashAccount.accountID>>,
						And<CashAccount.subID, Equal<Required<CashAccount.subID>>,
							And<CashAccount.branchID, Equal<Required<CashAccount.branchID>>>>>>.Select(this, newSplit.AccountID, newSplit.SubID, newSplit.BranchID);
					if (cashAccount == null)
					{
						string branchCD = (string)PXSelectorAttribute.GetField(sender, newSplit, typeof(CABankTranDetail.branchID).Name, newSplit.BranchID, typeof(Branch.branchCD).Name);
						sender.RaiseExceptionHandling<CABankTranDetail.branchID>(newSplit, branchCD,
							new PXSetPropertyException(Messages.NoCashAccountForBranchAndSub, PXErrorLevel.Error));
						string subCD = (string)PXSelectorAttribute.GetField(sender, newSplit, typeof(CABankTranDetail.subID).Name, newSplit.SubID, typeof(Sub.subCD).Name);
						sender.RaiseExceptionHandling<CABankTranDetail.subID>(newSplit, subCD,
							new PXSetPropertyException(Messages.NoCashAccountForBranchAndSub, PXErrorLevel.Error));
						e.Cancel = true;
						return;
					}
		...
		private CABankTranDetail GetDefaultAccountValues(PXGraph graph, int? CashAccountID, string EntryTypeID)
		{
			...
			CABankTranDetail result = new CABankTranDetail();
		...
		private bool ProcessCATran(CABankTran det, List<Batch> externalPostList, Int64? tranID, bool checkAmt = true)
		{
			...
			if (tran != null)
			{
				...
				if (tran.Released == true && tran.Cleared == false)
				{
					...
					CABankStatementEntry.UpdateSourceDoc(this, tran, det.TranDate);
				}
		...
		protected virtual PXResult<APInvoice, APAdjust, APPayment> FindAPInvoiceByInvoiceInfo(CABankTran aRow)
		{
			...
			PXResult<APInvoice, APAdjust, APPayment> invResult = (PXResult<APInvoice, APAdjust, APPayment>)PXSelectJoin<APInvoice,
				...
				//												And<APInvoice.curyDocBal, Equal<Required<APInvoice.curyDocBal>>,
		...
		public virtual void DoMatch(PXCache aUpdateCache, IEnumerable<CABankTran> aRows)
		{
			...
			{
				if ((det.DocumentMatched != true && det.CreateDocument != true) && (det.CountMatches.HasValue && det.CountMatches == 0 && det.CountInvoiceMatches.HasValue && det.CountInvoiceMatches == 0))
				{
		...
		protected virtual IEnumerable FindDetailMatchingInvoices(CABankTran aDetail, MatchSettings aSettings, decimal aRelevanceTreshold, CABankTranInvoiceMatch[] aBestMatches)
		{
			...
			if (aDetail.DrCr == CA.CADrCr.CADebit && amount > 0)
			{
				foreach (PXResult<Light.ARInvoice, Light.BAccount, CABankTranMatch, Light.ARAdjust, Light.CABankTranAdjustment> it in 
					...
					Where<Light.ARAdjust.adjgRefNbr, IsNull,
						...
						And<Light.ARInvoice.docType, Equal<ARDocType.invoice>,
						And<Light.ARInvoice.curyID, Equal<Required<ARInvoice.curyID>>,
						And<Light.ARInvoice.curyDocBal, Equal<Required<ARInvoice.curyDocBal>>>>>>>>>.Select(this, aDetail.TranType, aDetail.CuryID, amount, aDetail.TranDate))
				{
			...
			{
				if (aDetail.DrCr == CA.CADrCr.CACredit)
				{

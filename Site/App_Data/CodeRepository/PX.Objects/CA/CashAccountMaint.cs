...
namespace PX.Objects.CA
{
    ...
    public class CashAccountMaint : PXGraph<CashAccountMaint, CashAccount>
	{
		...
		public PXSelect<CashAccountCheck, Where<CashAccountCheck.accountID, Equal<Optional<CashAccount.accountID>>>> CashAccountChecks; 
        ...
        public IEnumerable paymentMethodForRemittance() 
        {
            ...
            {
				PaymentMethod pm= PXSelect<PaymentMethod,Where<PaymentMethod.paymentMethodID, Equal<Required<PaymentMethod.paymentMethodID>>>>.Select(this,it.PaymentMethodID);
        ...
        protected virtual void CashAccount_RowDeleted(PXCache sender, PXRowDeletedEventArgs e)
        {
			...
			foreach (CashAccountCheck check in CashAccountChecks.Select(cashAccount.AccountID))
			{
        ...
        protected virtual void CashAccount_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
        {
			...
			if (CashAccount.Cache.GetStatus(cashacct) == PXEntryStatus.Inserted)
			{
				using (new PXReadBranchRestrictedScope())
				{
					...
					if (e1 != null)
					{
						CashAccount.Cache.RaiseExceptionHandling<CashAccount.cashAccountCD>(cashacct, cashacct.CashAccountCD, new PXSetPropertyException(CA.Messages.CashAccountExists));
						throw new PXSetPropertyException(CA.Messages.CashAccountExists);
					}
				}
			}
		...
		protected virtual void CashAccount_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
        {
			...
			if (!sender.ObjectsEqual<CashAccount.accountID>(updatedCashAccount, oldCashAccount))
			{
				...
				foreach (CashAccount cashAccount in accountsToProcess)
				{
					...
					if (generalLedgerAccount != null && generalLedgerAccount.IsCashAccount != !isLastCashAccount)
					{
						generalLedgerAccount = PXCache<Account>.CreateCopy(generalLedgerAccount);
						generalLedgerAccount.IsCashAccount = !isLastCashAccount;
						...
						// -
						if (!isLastCashAccount && generalLedgerAccount.PostOption != GL.AccountPostOption.Detail)
						{
        ...
        protected virtual void CashAccount_ClearingAccount_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
        {
            ...
            if ((bool)e.NewValue == true)
            {
                ...
                bool hasPaymentType = this.Details.Any();
                bool hasAP = false;
        ...
        public override void Persist()
        {
            ...
            {
                if (!IsCorrectCashAccount(this.ETDetails.Cache, etDet)) existOffsetCashAccountErrors = true;
            }
        ...
        private CashAccountOptions DetectOptions()
        {
            ...
            foreach (PXResult<PaymentMethodAccount, PaymentMethod> iPt in this.Details.Select())
            {
                ...
                if (AcctSettingsAllowed == false)
                {
					...
					if (ptd != null)
					{
						PaymentMethod pm=PXSelect<PaymentMethod, Where<PaymentMethod.paymentMethodID, Equal<Required<PaymentMethod.paymentMethodID>>>>.Select(this, ptd.PaymentMethodID);
						if (pm != null && pm.UseForCA == true)

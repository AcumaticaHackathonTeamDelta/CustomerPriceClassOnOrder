...
namespace PX.Objects.AR
{
				...
				Or<BAccount.type, Equal<BAccountType.combinedType>>>>
	{
		...
		[PXHidden]
		[CRReference(typeof(Select<Customer, Where<Customer.bAccountID, Equal<Current<Customer.bAccountID>>>>), Persistent = true)]
		...
		public IEnumerable defPaymentMethodInstanceDetails()
		{
			...
			if (currCPM != null)
			{
				bool isTokenized = CCProcessingUtils.isTokenizedPaymentMethod(this, currCPM.PMInstanceID);
				bool isHF = CCProcessingUtils.isHFPaymentMethod(this, currCPM.PMInstanceID);
				return CCProcessingUtils.getPMdetails(this, currCPM.PMInstanceID, isTokenized, !isTokenized);
			}
			else
			{
				return null;
		...
		public virtual IEnumerable RegenerateLastStatement(PXAdapter adapter)
		{
			...
			var statements = PXSelect<ARStatement, Where<ARStatement.customerID, Equal<Required<Customer.bAccountID>>,
														...
														.Select(this, customer.BAccountID, customer.BAccountID, cycle.StatementCycleId);
		...
		public virtual IEnumerable CustomerStatement(PXAdapter adapter)
		{
			...
			var parameters = ARStatementReportParams.FromCustomer(customer);
			parameters[ARStatementReportParams.Parameters.StatementDate] = Convert.ToString(cycle.LastStmtDate.Value.Date, System.Globalization.CultureInfo.InvariantCulture);

			string reportID = ARStatementReportParams.ReportIDForCustomer(this, customer, null);
			throw new PXReportRequiredException(parameters, reportID, Messages.CustomerStatement);
		}
            ...
            return PXSelectReadonly2<
                Override.Customer,
                    InnerJoin<Override.BAccount,
                        On<Override.Customer.bAccountID, Equal<Override.BAccount.bAccountID>>>,
                Where<
                    Override.BAccount.bAccountID, Equal<Required<Customer.bAccountID>>,
                    Or<ParentField, Equal<Required<ParentField>>>>>
                .Select(graph, customerID, customerID)
                .Cast<PXResult<Override.Customer, Override.BAccount>>()
                .Select(result => new Override.ExtendedCustomer(result, result));
        }
		...
		protected virtual void Customer_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			extendToVendor.SetEnabled(!((row.Type == BAccountType.VendorType || row.Type == BAccountType.CombinedType) && status != PXEntryStatus.Inserted));
			...
			if (this.DefLocation.Current != null && this.CustomerClass.Current != null)
			{
				...
				PXDefaultAttribute.SetPersistingCheck<LocationExtAddress.cAvalaraCustomerUsageType>(this.DefLocation.Cache, this.DefLocation.Current, isRequiredUsageType ? PXPersistingCheck.Null : PXPersistingCheck.Nothing);
				PXUIFieldAttribute.SetRequired<LocationExtAddress.cAvalaraCustomerUsageType>(this.DefLocation.Cache, isRequiredUsageType);
				if (isRequiredUsageType && string.IsNullOrEmpty(this.DefLocation.Current.CAvalaraCustomerUsageType))
				{
					if (this.DefLocation.Cache.GetStatus(this.DefLocation.Current) == PXEntryStatus.Notchanged)
					{
						this.DefLocation.Cache.SetStatus(this.DefLocation.Current, PXEntryStatus.Updated);
			...
			newInvoiceMemo.SetEnabled(row.Status != CR.BAccount.status.Hold && row.Status != CR.BAccount.status.CreditHold && row.Status != CR.BAccount.status.Inactive);
			newSalesOrder.SetEnabled(row.Status != CR.BAccount.status.Hold && row.Status != CR.BAccount.status.CreditHold && row.Status != CR.BAccount.status.Inactive);
			newPayment.SetEnabled(row.Status != CR.BAccount.status.Hold && row.Status != CR.BAccount.status.Inactive);
		}
		...
		protected virtual void Customer_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
		{
			...
			if (e.Operation != PXDBOperation.Delete)
			{
				...
				if (row.ParentBAccountID != null &&
					...
					row.StatementType == StatementTypes.CS_BALANCE_BROUGHT_FORWARD)
				{
					...
					&& String.IsNullOrEmpty(PXUIFieldAttribute.GetError<Customer.parentBAccountID>(cache, row)))
				{
					...
					if(oldParent != null && oldParent.StatementType == StatementTypes.CS_BALANCE_BROUGHT_FORWARD
					|| newParent != null && newParent.StatementType == StatementTypes.CS_BALANCE_BROUGHT_FORWARD
					|| row.StatementType == StatementTypes.CS_BALANCE_BROUGHT_FORWARD)
					{
		...
		private void RaiseEmailErrors(PXCache persistingCache, PXCache contactCache, Contact contact, Customer customer)
		{
			...
			if (customer.MailInvoices == true)
			{
				...
				persistingCache.RaiseExceptionHandling<Customer.mailInvoices>(customer, customer.SendStatementByEmail, 
					new PXSetPropertyException(Messages.ERR_EmailIsRequiredForOption, PXErrorLevel.RowError, invoicesFieldName));
			}
			if (customer.StatementChild != true && customer.SendStatementByEmail == true)
			{
				...
				persistingCache.RaiseExceptionHandling<Customer.sendStatementByEmail>(customer, customer.SendStatementByEmail, 
					new PXSetPropertyException(Messages.ERR_EmailIsRequiredForOption, PXErrorLevel.RowError, statementsFieldName));
			}
			if (customer.SharedCreditChild != true && customer.MailDunningLetters == true && PXAccess.FeatureInstalled<FeaturesSet.dunningLetter>())
			{
				...
				persistingCache.RaiseExceptionHandling<Customer.mailDunningLetters>(customer, customer.MailDunningLetters, 
					new PXSetPropertyException(Messages.ERR_EmailIsRequiredForOption, PXErrorLevel.RowError, dunningLetterFieldName));
			}

			if (!ok)
			{
				contactCache.RaiseExceptionHandling<Contact.eMail>(contact, contact.EMail,
					new PXSetPropertyException(Messages.ERR_EmailIsRequiredForSendByEmailOptions, PXErrorLevel.RowError, allFields));
				throw new PXSetPropertyException(Messages.ERR_EmailIsRequiredForSendByEmailOptions, PXErrorLevel.RowError, allFields);
			}
			}
				...
				if (acct != null && acct.DefPMInstanceID == row.PMInstanceID)
				{
					...
					if (!String.IsNullOrEmpty(row.PaymentMethodID))
					{
						...
						bool customerIdVisisble = isInserted && pmDef.PaymentType == PaymentMethodType.CreditCard
												  && CCProcessingUtils.isTokenizedPaymentMethod(this, row.PMInstanceID);

...
namespace PX.Objects.AR
{
				...
				try
				{
					...
					try
					{
						...
						if (childs != null)
						{
							...
							{
								rg.Clear();
								rg.ReleaseDocProc(je, child, pmBatchList);
	...
	public class ARReleaseProcess : PXGraph<ARReleaseProcess>
	{
		...
		{
			PXCache cache = Caches[typeof(ARBalances)];

			ARBalances arbal = new ARBalances();
			arbal.BranchID = invoice.BranchID;
			arbal.CustomerID = invoice.CustomerID;
			arbal.CustomerLocationID = invoice.CustomerLocationID;
			arbal = (ARBalances)cache.Insert(arbal);

			if (invoice.DueDate != null && (arbal.OldInvoiceDate == null || ((DateTime)invoice.DueDate) <= ((DateTime)arbal.OldInvoiceDate)))
			{
				if ((bool)invoice.OpenDoc)
				{
			...
			if ((bool)doc.Released == false)
			{
				...
				else
				{
					...
					if (arAccount != null && arAccount.AccountGroupID != null && ardoc.ProjectID != null && !ProjectDefaultAttribute.IsNonProject(this, ardoc.ProjectID))
					{
						...
						if (mapping == null)
						{
							throw new PXException(Messages.TaxAccountTaskMappingNotFound);
						}
						...
						if (!_IsIntegrityCheck)
						{
							...
							if (n.InventoryID != null && (n.LineType == null || ((n.LineType == SO.SOLineType.Inventory || n.LineType == SO.SOLineType.NonInventory) && !tranCostSet) || n.LineType == SO.SOLineType.MiscCharge))
							{
								...
								if (result != null)
								{
									...
									else if (n.SOShipmentType != SO.SOShipmentType.DropShip)
									{
		...
		public virtual void UpdateBalances(ARAdjust adj, ARRegister adjddoc)
		{
			...
			if (ardoc.CuryDocBal == 0m)
			{
				...
				foreach (ARAdjust adjmax in PXSelectGroupBy<ARAdjust, Where<ARAdjust.adjgDocType, Equal<Required<ARAdjust.adjgDocType>>, And<ARAdjust.adjgRefNbr, Equal<Required<ARAdjust.adjgRefNbr>>, And<Where<ARAdjust.released, Equal<True>, Or<ARAdjust.adjNbr, Equal<Required<ARAdjust.adjNbr>>>>>>>, Aggregate<GroupBy<ARAdjust.adjgDocType, GroupBy<ARAdjust.adjgRefNbr, Max<ARAdjust.adjgFinPeriodID, Max<ARAdjust.adjgTranPeriodID>>>>>>.Select(this, ardoc.DocType, ardoc.RefNbr, adj.AdjNbr))
				{
		...
		private void UpdateVoidedCheck(ARPayment voidcheck, string OrigDocType)
		{
			foreach (PXResult<ARPayment, CurrencyInfo, Currency, Customer> res in ARPayment_DocType_RefNbr.Select(OrigDocType, voidcheck.RefNbr, voidcheck.CustomerID))
			{
				...
				PXDatabase.Update<SO.SOAdjust>(new PXDataFieldRestrict(typeof(SO.SOAdjust.adjgDocType).Name, PXDbType.VarChar, SO.SOAdjust.AdjgDocTypeLength, ardoc.DocType, PXComp.EQ),
		...
		public virtual void ReleaseDocProc(JournalEntry je, ref ARRegister doc, PXResult<ARPayment, CurrencyInfo, Currency, Customer, CashAccount> res, int? AdjNbr)
		{
			...
			if (doc.Released == false)
			{
				...
				{
					bool isCADebit = charge.DrCr == DrCr.Debit || doc.DocType == ARDocType.CashReturn;
			...
			PXResultset<ARAdjust> adjustments = ARAdjust_AdjgDocType_RefNbr_CustomerID.Select(doc.DocType, doc.RefNbr, AdjNbr);
				...
				(doc.CuryDocBal == 0m && doc.DocBal != 0m || isOpenPaymentWithNegativeBalance))
			{
				...
				UpdateARBalances(this, doc, doc.DocBal, 0m);
				...
				var adjdDoc = (ARRegister)ARDocument.Cache.Locate(new ARRegister { DocType = prev_adj.AdjdDocType, RefNbr = prev_adj.AdjdRefNbr });
				if (adjdDoc != null)
				{
			...
			if (ardoc.SelfVoidingDoc == true && prev_adj.Voided == true)
			{
				UpdateVoidedCheck(ardoc, ardoc.DocType);
			}
			...
			if ((bool)ardoc.VoidAppl || doc.CuryDocBal == 0m)
			{
				...
				if (doc.ClosedFinPeriodID == null || doc.ClosedTranPeriodID == null)
				{
					doc.ClosedTranPeriodID = doc.TranPeriodID;
					doc.ClosedFinPeriodID = doc.FinPeriodID;
				}
		...
		protected virtual Tuple<ARAdjust, CurrencyInfo> ProcessAdjustments(JournalEntry je, PXResultset<ARAdjust> adjustments, ARRegister paymentRegister, ARPayment payment, Customer paymentCustomer, CurrencyInfo new_info, Currency paycury)
		{
			...
			foreach (PXResult<ARAdjust, CurrencyInfo, Currency, ARRegister, ARPayment> adjres in adjustments)
			{
				...
				ProcessSVATAdjustments(adj, adjustedInvoice, paymentRegister);
			}
		...
		public virtual void ReleaseSmallCreditProc(JournalEntry je, ref ARRegister doc, PXResult<ARInvoice, CurrencyInfo, Terms, Customer> res)
		{
			...
			foreach (PXResult<ARAdjust, CurrencyInfo, Currency, ARPayment> adjres in ARAdjust_AdjdDocType_RefNbr_CustomerID.Select(doc.DocType, doc.RefNbr, doc.AdjCntr))
			{
				ARAdjust adj = adjres;
				decimal? RGOLAmt = adj.RGOLAmt;
				//RGOLAmt should be zero for correct ARBalance calculation
				adj.RGOLAmt = 0m;
				...
				if (RGOLAmt != 0m)
				{
					...
					UpdateARBalances(this, doc, RGOLAmt, 0m);

					//BaseCalc should be false
					prev_adj.AdjAmt += RGOLAmt;
					prev_adj.RGOLAmt -= RGOLAmt;
					prev_adj = (ARAdjust)Caches[typeof(ARAdjust)].Update(prev_adj);

					bool isLoss = RGOLAmt < 0m;

					tran = new GLTran();
					tran.SummPost = true;
					tran.BranchID = ardoc.BranchID;
					tran.AccountID = isLoss 
						? cury.RoundingLossAcctID 
						: cury.RoundingGainAcctID;
					tran.SubID = isLoss
						? GainLossSubAccountMaskAttribute.GetSubID<Currency.roundingLossSubID>(je, tran.BranchID, cury)
						: GainLossSubAccountMaskAttribute.GetSubID<Currency.roundingGainSubID>(je, tran.BranchID, cury);
					tran.OrigAccountID = adjddoc.ARAccountID;
					tran.OrigSubID = adjddoc.ARSubID;
					tran.CreditAmt = isLoss ? 0m : RGOLAmt;
					tran.CuryCreditAmt = 0m;
					tran.DebitAmt = isLoss ? -RGOLAmt : 0m;
					tran.CuryDebitAmt = 0m;
					tran.TranType = adj.AdjdDocType;
					tran.TranClass = GLTran.tranClass.Payment;
					tran.RefNbr = adj.AdjdRefNbr;
					...
					tran.SubID = adjddoc.ARSubID;
					tran.ReclassificationProhibited = true;
					tran.DebitAmt = isLoss ? 0m : RGOLAmt;
					tran.CuryDebitAmt = 0m;
					tran.CreditAmt = isLoss ? -RGOLAmt : 0m;
					tran.CuryCreditAmt = 0m;
					tran.TranType = adj.AdjdDocType;
					tran.TranClass = GLTran.tranClass.Normal;
					tran.RefNbr = adj.AdjdRefNbr;
				...
				ProcessSVATAdjustments(adj, doc, adjddoc);
			}
				...
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					foreach (PXResult<ARPayment, CurrencyInfo, Currency, Customer, CashAccount> res in ARPayment_DocType_RefNbr.Select((object)doc.DocType, doc.RefNbr, doc.CustomerID))
					{
						...
						if ((doc.DocType == ARDocType.Payment || doc.DocType == ARDocType.VoidPayment || doc.DocType == ARDocType.Refund) && doc.Released == false)
						{
							...
							{
								ARAdjust_AdjgDocType_RefNbr_CustomerID.StoreCached(new PXCommandKey(new object[] { doc.DocType, doc.RefNbr, i }), pair.Value);
								JournalEntry.SegregateBatch(je, BatchModule.AR, doc.BranchID, doc.CuryID, datesbyperiod[pair.Key], pair.Key, doc.DocDesc, (CurrencyInfo)res, paymentBatch);

...
namespace PX.Objects.AR
{
	public class CustomerPaymentMethodMaint : PXGraph<CustomerPaymentMethodMaint, CustomerPaymentMethod>
	{
		...
		{
			CCCustomerInformationManager.CreatePaymentMethodHF(this, CustomerPaymentMethod, CustomerPaymentMethod.Current);
			return adapter.Get();
		}
		...
		public virtual IEnumerable SyncCCPaymentMethods(PXAdapter adapter)
		{
			...
			bool isIDFilled = CCProcessingUtils.isCCPIDFilled(this, CustomerPaymentMethod.Current.PMInstanceID);
			if (!isIDFilled)
			{
				CCCustomerInformationManager.SyncPaymentMethodsHF(this, CustomerPaymentMethod, DetailsAll, CustomerPaymentMethod.Current);
			}
			else
			{
				CCCustomerInformationManager.SyncExistingPMI(this, CustomerPaymentMethod, DetailsAll);
			}
			this.Save.Press();
			return cpmList;
		}
		...
		{
			CCCustomerInformationManager.ManagePaymentMethodHF(this, CustomerPaymentMethod.Current);
			return adapter.Get();
		}
		...
		{
			bool isTokenized = CCProcessingUtils.isTokenizedPaymentMethod(this, CustomerPaymentMethod.Current.PMInstanceID);
			bool isHF = CCProcessingUtils.isHFPaymentMethod(this, CustomerPaymentMethod.Current.PMInstanceID);
			...
			return CCProcessingUtils.getPMdetails(this, CustomerPaymentMethod.Current.PMInstanceID, showToken, showDetails);
		}
		...
		protected virtual void CustomerPaymentMethodInputMode_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			{
				bool isTokenized = CCProcessingUtils.isTokenizedPaymentMethod(this, CustomerPaymentMethod.Current.PMInstanceID);
				bool isHFPM = CCProcessingUtils.isHFPaymentMethod(this, CustomerPaymentMethod.Current.PMInstanceID);
		...
		protected virtual void CustomerPaymentMethod_RowUpdated(PXCache cache, PXRowUpdatedEventArgs e)
		{
			...
			if (!cache.ObjectsEqual<CustomerPaymentMethod.isActive>(e.OldRow, e.Row))
			{
				if (row.IsActive == false)
				{
		...
		protected virtual void CustomerPaymentMethod_RowDeleting(PXCache cache, PXRowDeletingEventArgs e)
		{
			...
			if (row != null)
			{
				...
				if (CCPaymentEntry.HasUnfinishedCCTrans(this, row))
				{
		...
		protected virtual void CustomerPaymentMethodDetail_Value_FieldVerifying(PXCache cache, PXFieldVerifyingEventArgs e)
		{
			...
			if (def != null)
			{
				...
				{
					PXResultset<CustomerPaymentMethodDetail> otherCards = CCCustomerInformationManager.GetAllCustomersCardsInProcCenter(this, CustomerPaymentMethod.Current.BAccountID, 
						CustomerPaymentMethod.Current.CCProcessingCenterID);

					PXResult<CustomerPaymentMethodDetail> duplicate = otherCards
						.Where(result => ((CustomerPaymentMethodDetail)result).Value == (string)e.NewValue)
						.FirstOrDefault();
		...
		public static DateTime? ParseExpiryDate(PXGraph graph, CustomerPaymentMethod cpm, string aValue)
		{
			...
			{
				dateStringFormat = CCProcessingUtils.GetExpirationDateFormat(graph, cpm.CCProcessingCenterID);
			}
		...
		public override void Persist()
		{
			...
			if (!isCPMDeleting && isDetailsInserting && !string.IsNullOrEmpty(CustomerPaymentMethod.Current.CCProcessingCenterID)
				&& CCProcessingUtils.isTokenizedPaymentMethod(this, CustomerPaymentMethod.Current.PMInstanceID))
			{
				CCCustomerInformationManager.SyncNewPMI(this, CustomerPaymentMethod, DetailsAll);
			}
			//assuming only one record can be deleted from promary view of this graph at a time
			else if (isCPMDeleting && CCProcessingUtils.isTokenizedPaymentMethod(this, null))
			{
				CCCustomerInformationManager.SyncDeletePMI(this, CustomerPaymentMethod, Details);
			}
			base.Persist();
		}

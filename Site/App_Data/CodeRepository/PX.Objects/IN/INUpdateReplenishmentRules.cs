...
namespace PX.Objects.IN
{
	...
	public class INUpdateReplenishmentRules : PXGraph<INUpdateReplenishmentRules>
	{
			...
			public static class Actions
			{
				...
				public class ActionsListAttribute : PXStringListAttribute
				{
					public ActionsListAttribute()
						: base(
						new string[] { Calculate, Clear },
						new string[] { Messages.Calculate, Messages.Clear }) { }
				}
		...
		public IEnumerable records() 
		{
			...
			if (filt.Action == Filter.Actions.Calculate)
			{
				...
				if (String.IsNullOrEmpty(filt.ItemClassID) == false)
				{
					select.WhereAnd<Where<InventoryItemRO.itemClassID, Equal<Current<Filter.itemClassID>>>>();
				}
	...
	public class INRepForecastApplicationGraph : PXGraph<INRepForecastApplicationGraph>
	{
		...
		public PXCancel<Filter> Cancel;
		[PXFilterable]
		public PXFilteredProcessing<INItemSite, Filter, Where<INItemSite.siteID, Equal<Current<INItemSite.siteID>>,
                And2<Where<
				...
				And<Where<INItemSite.lastFCApplicationDate, IsNull, 
					Or<INItemSite.lastFCApplicationDate, Less<INItemSite.lastForecastDate>>>>>>>> Records;
		public IEnumerable records() 
		{
			foreach (INItemSite it in PXSelect<INItemSite, Where<INItemSite.siteID, Equal<Current<Filter.siteID>>,
				And<INItemSite.lastForecastDate, IsNotNull,
				And<Where<INItemSite.lastFCApplicationDate, IsNull,

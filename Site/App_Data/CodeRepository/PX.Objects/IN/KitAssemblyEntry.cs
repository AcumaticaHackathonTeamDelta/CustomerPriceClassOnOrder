...
namespace PX.Objects.IN
{
	...
	public partial class INKitRegister : IBqlTable, ILSPrimary
	{
		...
		protected DateTime? _ExpireDate;
		[INExpireDate(typeof(INKitRegister.inventoryID), PersistingCheck = PXPersistingCheck.Nothing, BqlField=typeof(INTran.expireDate))]
		public virtual DateTime? ExpireDate
		{
			get
			{
	    ...
	    And<INKitTranSplit.refNbr, Equal<Current<INKitRegister.refNbr>>>>>
	{
		...
		protected override void RaiseQtyExceptionHandling(PXCache sender, object row, object newValue, PXSetPropertyException e)
		{
			...
			{
				sender.RaiseExceptionHandling<INKitRegister.qty>(row, null, new PXSetPropertyException(e.MessageNoPrefix, PXErrorLevel.Warning, sender.GetStateExt<INKitRegister.inventoryID>(row), sender.GetStateExt<INKitRegister.subItemID>(row), sender.GetStateExt<INKitRegister.siteID>(row), sender.GetStateExt<INKitRegister.locationID>(row), sender.GetValue<INKitRegister.lotSerialNbr>(row)));
			}
			else
			{

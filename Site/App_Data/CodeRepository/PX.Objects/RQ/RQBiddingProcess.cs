	...
	public class RQBiddingProcess : PXGraph<RQBiddingProcess>
	{
		...
		private RQRequisition DoChooseVendor(RQRequisition item, RQBiddingVendor vendor)
		{
            ...
            foreach (RQRequisitionLine line in this.Lines.View.SelectMultiBound(new object[] { upd }))
            {
                ...
                if (bidding != null && 
                    ((bidding.MinQty == 0 && bidding.QuoteQty ==0) ||
                     (bidding.MinQty <= line.OrderQty && bidding.QuoteQty >= line.OrderQty)))
                    l.CuryEstUnitCost = bidding.CuryQuoteUnitCost;
                else
                {
                    Decimal unitCost;
                    PXCurrencyAttribute.CuryConvCury<RQRequisitionLine.curyInfoID>
                        (this.Lines.Cache, l, l.EstUnitCost ?? 0, out unitCost);
                    l.CuryEstUnitCost = unitCost;
                }
				...
				if (item.Splittable == true)
				{
					if (item.VendorID == null || item.VendorLocationID == null)
					{
						...
						foreach (RQRequisitionLine line in this.Lines.Select(item.ReqNbr))
						{
							...
							if(vendorLocationID == null || vendorLocationID > 0 )							
								foreach (RQBidding b in PXSelect<RQBidding,
										Where<RQBidding.reqNbr, Equal<Required<RQBidding.reqNbr>>,
											And<RQBidding.lineNbr, Equal<Required<RQBidding.lineNbr>>,
											And<RQBidding.orderQty, Greater<CS.decimal0>>>>>.Select(this, line.ReqNbr, line.LineNbr))
								{
									if (b.OrderQty > 0)
									{

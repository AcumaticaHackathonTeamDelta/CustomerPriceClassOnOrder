...
namespace PX.Objects.CM
{
	...
	public class TranslationProcess : PXGraph<TranslationProcess>
	{
		...
		public partial class TranslationParams : PX.Data.IBqlTable
		{
			...
			[PXDefault(typeof(CMSetup.translDefId))]
			[PXUIField(DisplayName = "Translation ID", Required = true)]
			[PXSelector(typeof(Search<TranslDef.translDefId, Where<TranslDef.active, Equal<boolTrue>>>), DescriptionField = typeof(TranslDef.description))]
		...
		public bool translAvailable = true;
		#endregion
		...
		{
			TranslationParams par = (TranslationParams)e.Row;
			if (par == null) return;

			FinPeriod curPer = (FinPeriod)PXSelect<FinPeriod, Where<FinPeriod.finPeriodID, Equal<Required<FinPeriod.finPeriodID>>>>.Select(cache.Graph, e.NewValue);
			if (curPer != null)
			{
				if (curPer.Active != true)
				{
					translAvailable = false;
					cache.RaiseExceptionHandling<TranslationParams.finPeriodID>(par, e.NewValue, new PXSetPropertyException(Messages.FuturePeriodIsNotAvalableForTranslation));
				}
				else
				{
					translAvailable = true;
				}
			}
			else
			{
				translAvailable = false;
				cache.RaiseExceptionHandling<TranslationParams.finPeriodID>(par, e.NewValue, new PXSetPropertyException(GL.Messages.NoPeriodsDefined));
			}
		}
		...
		protected virtual void TranslationParams_CuryEffDate_FieldVerifying(PXCache cache, PXFieldVerifyingEventArgs e)
		{
			TranslationParams par = (TranslationParams)e.Row;
			if (par == null) return;
			DateTime curyEffDate = (DateTime)e.NewValue;

			FinPeriod curPer = (FinPeriod)PXSelect<FinPeriod, Where<FinPeriod.finPeriodID, Equal<Required<FinPeriod.finPeriodID>>>>.Select(cache.Graph, par.FinPeriodID);
			if (curPer != null)
			{
				if (!(curyEffDate >= curPer.StartDate && curyEffDate <= curPer.EndDate))
				{
					translAvailable = false;
					cache.RaiseExceptionHandling<TranslationParams.curyEffDate>(par, e.NewValue, new PXSetPropertyException(Messages.DateNotBelongFinancialPeriod));
				}
			}
		}
		...
		{
			Translate.SetEnabled(translAvailable);
		}
		#endregion

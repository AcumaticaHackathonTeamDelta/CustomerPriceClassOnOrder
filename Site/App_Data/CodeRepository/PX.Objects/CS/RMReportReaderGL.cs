...
namespace PX.Objects.CS
{
	public class RMReportReaderGL : PXGraphExtension<RMReportReader>
    {
        ...
        public virtual object GetHistoryValue(ARmDataSet dataSet, bool drilldown, Func<ARmDataSet, bool, object> del)
        {
            ...
            if (rmType == ARmReport.GL)
            {
                ...
                !string.IsNullOrEmpty(dataSet[Keys.AccountClass] as string ?? "")) && !string.IsNullOrEmpty(dataSet[Keys.StartPeriod] as string ?? "") && (short?)dataSet[Keys.AmountType] != (short?)BalanceType.NotSet)))
                {
                    ...
                    List<Account> accounts = _accountRangeCache.GetItemsInRange(dataSet[Keys.StartAccount] as string,
                        range => range + dataSet[Keys.AccountClass] as string ?? "",
                        range => GetAccountsInRange(range, dataSet[Keys.AccountClass] as string ?? ""));
                    List<Sub> subs = _subRangeCache.GetItemsInRange(dataSet[Keys.StartSub] as string, 
                        sub=> sub.SubCD,
						(sub, code) => sub.SubCD = code);
                    List<Branch> branchList = _branchRangeCache.GetItemsInRange(dataSet[Keys.StartBranch] as string,
                        branch => branch.BranchCD,
						(branch, code) => branch.BranchCD = code);

                    if (ds.Expand == ExpandType.Account)
                    {
                        accounts.ForEach(a => splitret.Add(new object[] { a.AccountCD, a.Description, 0m, null, string.Empty }));
                    }
                    else if (ds.Expand == ExpandType.Sub)
                    {
                        subs.ForEach(s => splitret.Add(new object[] { s.SubCD, s.Description, 0m, null, string.Empty }));
                    }
        ...
        private GLHistoryByPeriod GetGLHistoryByPeriodRecordForDrilldown(RMDataSource ds, GLHistory hist)
        {
            ...
            ghp.LastActivityPeriod = FinPeriodIDFormattingAttribute.FormatForStoring(ds.AmountType.ToString() + RMReportConstants.RangeDelimiterChar.ToString() + Base.DrilldownNumber.ToString());
            return ghp;
        }
        ...
        public virtual List<ARmUnit> ExpandUnit(RMDataSource ds, ARmUnit unit, Func<RMDataSource, ARmUnit, List<ARmUnit>> del)
        {
            if (unit.DataSet.Expand == ExpandType.Account)
            {
                ...
                return RMReportUnitExpansion<Account>.ExpandUnit(Base, ds, unit, Keys.StartAccount, Keys.EndAccount, 
                    rangeToFetch => _accountRangeCache.GetItemsInRange(rangeToFetch[Keys.StartAccount] as string,
                        range => range + rangeToFetch[Keys.AccountClass] as string ?? "",
                        range => GetAccountsInRange(range, rangeToFetch[Keys.AccountClass] as string ?? "")),
            ...
            else if(unit.DataSet.Expand == ExpandType.Sub)
            {
                GLEnsureInitialized();
                return RMReportUnitExpansion<Sub>.ExpandUnit(Base, ds, unit, Keys.StartSub, Keys.EndSub, 
                    rangeToFetch => _subRangeCache.GetItemsInRange(rangeToFetch[Keys.StartSub] as string,
                        sub => sub.SubCD,
                        (sub, code) => sub.SubCD = code),

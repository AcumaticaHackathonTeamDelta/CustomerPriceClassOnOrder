...
namespace PX.Objects.CS.Email
{
	...
	public static class PXExchangeConversionHelper
	{
		...
		public static EmailAddressType ParceAddress(string address)
		{
			...
			{
				return new EmailAddressType() { EmailAddress = ((Mailbox)item).Address, Name = ((Mailbox)item).DisplayName };
			}
			return null;
		}
			...
			foreach (PX.Common.Mail.Address item in AddressList.Parse(address))
			{
				...
				if (group != null)
					foreach (Mailbox mailbox in group.Members)
						addresses.Add(new EmailAddressType() { EmailAddress = mailbox.Address, Name = mailbox.DisplayName });
				else addresses.Add(new EmailAddressType() { EmailAddress = ((Mailbox)item).Address, Name = ((Mailbox)item).DisplayName });
			}
		...
		public static string ParceAddresses(EmailAddressType[] addresses)
		{
			...
			return (new MailboxList(addresses.Select(m => new Mailbox(m.Name, m.EmailAddress)))).ToString();
		}
	}
	#endregion
}

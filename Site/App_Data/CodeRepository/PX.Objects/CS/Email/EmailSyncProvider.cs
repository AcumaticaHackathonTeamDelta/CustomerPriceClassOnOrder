...
namespace PX.Objects.CS.Email
{
	...
	public abstract class BaseExchangeSyncProvider : IDisposable
	{
		...
		public void LogResult(PXSyncResult result)
		{
			...
			if (result.Success)
			{
				string text = PXMessages.LocalizeFormatNoPrefix(Messages.EmailExchangeSyncSuccessful, result.OperationTitle, result.Direction.ToString(), result.Address, action, result.DisplayKey);
				LogEvent(new PXExchangeEvent(result.Address, EventLevel.Informational, text, result.Error) { Date = result.Date });
			}
			else
			{
				string text = PXMessages.LocalizeFormatNoPrefix(Messages.EmailExchangeSyncError, result.OperationTitle, result.Direction.ToString(), result.Address, action, result.DisplayKey);

...
namespace PX.Objects.FA
{
	...
	public class AssetGLTransactions : PXGraph<AssetGLTransactions>
	{
		...
		public static FixedAsset InsertAsset(PXGraph graph, int? _classID, int? _parentID, string _assetCD, string _assetTypeID,
			DateTime? _recDate, DateTime? _deprFromDate, decimal? _cost, decimal? _usefulLife, decimal? _qty, GLTran _gltran, IFALocation _loc, out int? assetID)
		{
			if(_assetCD == null)
				_assetCD = GetTempKey<FixedAsset.assetCD>(graph.Caches[typeof(FixedAsset)]);

			FixedAsset asset = (FixedAsset)graph.Caches[typeof(FixedAsset)].Insert(new FixedAsset
			{
			...
			FALocationHistory location = (FALocationHistory)graph.Caches[typeof(FALocationHistory)].Insert(new FALocationHistory
			{
				AssetID = asset.AssetID,
				BranchID = _loc.BranchID,
			...
			FADetails det = (FADetails)graph.Caches[typeof(FADetails)].Insert(new FADetails
			{
				AssetID = asset.AssetID,
				...
				ReceiptType = aptran != null ? aptran.ReceiptType : null,
			});
			...
			foreach (FABookBalance bal in PXSelect<FABookSettings, 
				...
				.Select(sett => (FABookBalance)graph.Caches[typeof(FABookBalance)].Insert(new FABookBalance
				{
					AssetID = asset.AssetID,
					ClassID = _classID,
					BookID = sett.BookID,

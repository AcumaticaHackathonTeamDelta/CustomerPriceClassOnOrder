...
namespace PX.Objects.AP
{
							...
							Or<BAccount.type, Equal<BAccountType.combinedType>>>>
	{
		...
		[PXHidden]
		[CRReference(typeof(Select<Vendor, Where<Vendor.bAccountID, Equal<Current<Vendor.bAccountID>>>>), Persistent = true)]
		...
		protected virtual void Vendor_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			viewRestrictionGroups.SetEnabled(row?.AcctCD != null);
            ...
            extendToCustomer.SetEnabled(!viewCustomer.GetEnabled());
			...
			PXUIFieldAttribute.SetVisible<Vendor.taxYearStartDate>(cache, null, row.EnableTaxStartDate == true);
			PXUIFieldAttribute.SetEnabled<Vendor.taxYearStartDate>(cache, null, row.TaxPeriodType != PX.Objects.TX.VendorTaxPeriodType.FiscalPeriod);
			...
			newBillAdjustment.SetEnabled(row.Status != CR.BAccount.status.Hold && row.Status != CR.BAccount.status.Inactive);
			newManualCheck.SetEnabled(row.Status != CR.BAccount.status.Hold && row.Status != CR.BAccount.status.HoldPayments && row.Status != CR.BAccount.status.Inactive);
		}
		...
		protected virtual void Vendor_TaxPeriodType_FieldUpdating(PXCache sender, PXFieldUpdatingEventArgs e)
		{
			Vendor row = e.Row as Vendor;
			if (row != null && (string)e.NewValue == PX.Objects.TX.VendorTaxPeriodType.FiscalPeriod)
			{
				DateTime[] datesList = generateDatesList();
				DateTime taxYearStartDate = datesList[0];
				FinYear finYear = PXSelect<FinYear, Where<FinYear.startDate, LessEqual<Required<FinYear.startDate>>>, OrderBy<Desc<FinYear.year>>>.Select(this, taxYearStartDate);
				
				bool listContainsDate = false;
				for (int i = 0; i < datesList.Length; i++)
				{
					if (datesList[i] == finYear.StartDate)
					{
				...
				if (listContainsDate)
				{
					sender.SetValueExt<Vendor.taxYearStartDate>(row, finYear.StartDate);
				}
				else
				{
					throw new PXSetPropertyException(Messages.CannotChangeToFiscalYear, 
						PXUIFieldAttribute.GetDisplayName<Vendor.taxPeriodType>(sender), 
						TX.Messages.FiscalPeriod,
						PXUIFieldAttribute.GetDisplayName<Vendor.taxYearStartDate>(sender),
						finYear.StartDate.Value.ToString("d", CultureInfo.InvariantCulture));
				}
			}
		}
		...
		protected virtual void Vendor_RowUpdating(PXCache cache, PXRowUpdatingEventArgs e)
		{
			const string VendorKey = "VendorAskTaxPeriodKey";
			Vendor vendor = e.NewRow as Vendor;

			if (vendor.TaxAgency == true)
			{
				bool taxYearStartDateChanged = !cache.ObjectsEqual<Vendor.taxYearStartDate>(e.Row, vendor);
				if (vendor.EnableTaxStartDate == true && (taxYearStartDateChanged || BAccount.View.GetAnswer(VendorKey) != WebDialogResult.None))
				{
					PXResultset<TaxPeriod> openPeriods = PXSelect<TaxPeriod, Where<TaxPeriod.vendorID, Equal<Current<Vendor.bAccountID>>, And<TaxPeriod.status, Equal<TaxPeriodStatus.open>,
						And<TaxPeriod.startDate, GreaterEqual<Required<TaxPeriod.startDate>>, And<TaxPeriod.endDate, Greater<Required<TaxPeriod.startDate>>>>>>,
						OrderBy<Asc<TaxPeriod.taxPeriodID>>>.Select(this, vendor.TaxYearStartDate, vendor.TaxYearStartDate);

					if (openPeriods.Count != 0)
					{
						TaxPeriod lastPeriod = PXSelect<TaxPeriod, Where<TaxPeriod.vendorID, Equal<Current<Vendor.bAccountID>>>,
							OrderBy<Desc<TaxPeriod.taxPeriodID>>>.SelectWindowed(this, 0, 1);
						string PeriodTypeExt = ((PXStringState) BAccount.Cache.GetValueExt<Vendor.taxPeriodType>(vendor)).ValueLabelDic[vendor.TaxPeriodType];
						if (BAccount.Ask(VendorKey, Messages.TaxPeriodAskHeader,
                            PXMessages.LocalizeFormatNoPrefix(Messages.TaxYearChange, PeriodTypeExt, vendor.TaxYearStartDate.Value.ToString("d", this.Culture)),
							MessageButtons.YesNo) == WebDialogResult.Yes)
						{
							foreach (TaxPeriod period in openPeriods)
							{
								taxperiods.Delete(period);
							}
						}
						else
						{
		...
		private DateTime[] generateDatesList()
		{
			PXResultset<TaxPeriod> openPeriods = PXSelect<TaxPeriod, Where<TaxPeriod.vendorID, Equal<Current<Vendor.bAccountID>>, And<TaxPeriod.status, Equal<TaxPeriodStatus.open>>>,
					OrderBy<Desc<TaxPeriod.startDate>>>.Select(this);
			PXResultset<TaxYear> taxyears = PXSelect<TaxYear, Where<TaxYear.vendorID, Equal<Current<Vendor.bAccountID>>>, OrderBy<Desc<TaxYear.year>>>.Select(this);
			TaxTranReport oldestTran = CurrentVendor.Current != null ? OldestNotReportedTaxTran.Select(CurrentVendor.Current.BAccountID) : null;
			bool hasNotReportedTrans = oldestTran != null;
			bool yearExists = taxyears.Count != 0;
			bool vandorHasOpenPeriods = openPeriods.Count != 0;
			DateTime[] dates;
			if (!yearExists)
			{
				dates = new DateTime[12];
				DateTime firstdate;

				if (CurrentVendor.Current != null && CurrentVendor.Current.TaxYearStartDate.HasValue)
				{
					firstdate = CurrentVendor.Current.TaxYearStartDate.Value;
				}
				else
				{
					firstdate = !hasNotReportedTrans ? new DateTime(Accessinfo.BusinessDate.Value.Year, Accessinfo.BusinessDate.Value.Month, 1) :
						new DateTime(oldestTran.TranDate.Value.Year, oldestTran.TranDate.Value.Month, 1);
				}

				dates[0] = firstdate;
				for (int i = 1; i < 12; i++)
				{
					dates[i] = firstdate.AddMonths((-1) * i);
				}
			}
			else if (vandorHasOpenPeriods)
			{
				TaxYear currentYear = taxyears[0];
				TaxPeriod currentYearLastPeriod = PXSelect<TaxPeriod, Where<TaxPeriod.vendorID, Equal<Current<Vendor.bAccountID>>,
					And<TaxPeriod.taxYear, Equal<Required<TaxPeriod.taxYear>>>>, OrderBy<Desc<TaxPeriod.startDate>>>.Select(this, currentYear.Year);
				DateTime yearEndDate = currentYearLastPeriod.EndDate.Value;
				List<DateTime> datesList = new List<DateTime>();
				datesList.Add(yearEndDate);
				for (int i = 0; i < openPeriods.Count; i++)
				{
					DateTime startDate = ((TaxPeriod)openPeriods[i]).StartDate.Value;
					if (startDate.Day == 1 && startDate != yearEndDate)
					{
			...
			else
			{
				...
				TaxPeriod lastPeriod = PXSelect<TaxPeriod, Where<TaxPeriod.vendorID, Equal<Current<Vendor.bAccountID>>>,
						OrderBy<Desc<TaxPeriod.startDate>>>.SelectWindowed(this, 0, 1);
				dates[0] = lastPeriod.EndDate.Value;
			}
			return dates;
		}

		private string[] generateDatesStringList()
		{
			DateTime[] dates = generateDatesList();
			string[] dateStrings = new string[dates.Length];
			for (int i = 0; i < dates.Length; i++)
			{
				dateStrings[i] = dates[i].ToString("d", CultureInfo.InvariantCulture);
			}
			return dateStrings;
		}

		protected virtual void Vendor_EnableTaxStartDate_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			Vendor vend = e.Row as Vendor;
			if (vend == null) return;
			if (vend.EnableTaxStartDate == false)
			{
				sender.SetValue<Vendor.taxYearStartDate>(vend, null);
			}
			else
			{
				sender.SetDefaultExt<Vendor.taxYearStartDate>(vend);
			}
		}

		protected virtual void Vendor_TaxYearStartDate_FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			Vendor vend = e.Row as Vendor;
			if (vend == null) return;
			if (vend.EnableTaxStartDate == true)
			{
			string[] dates = generateDatesStringList();
			PXDateListAttribute.SetList<Vendor.taxYearStartDate>(sender, vend, dates, dates);
			if (vend.TaxYearStartDate == null)
			{
				DateTime[] datesDT = generateDatesList();
				e.ReturnValue = datesDT[0];
			}
			PXResultset<TaxYear> years = PXSelectJoin<TaxYear, InnerJoin<TaxPeriod, On<TaxYear.year, Equal<TaxPeriod.taxYear>, 
					And<TaxYear.vendorID, Equal<TaxPeriod.vendorID>>>>,
				Where<TaxYear.vendorID, Equal<Current<Vendor.bAccountID>>>,
				OrderBy<Desc<TaxPeriod.startDate>>>.Select(this);
			if (years.Count != 0)
			{
				PXResult<TaxYear, TaxPeriod> lastYearPeriod = (PXResult<TaxYear, TaxPeriod>) years[0];
				TaxYear lastYear = lastYearPeriod;
				TaxPeriod lastPeriod = lastYearPeriod;
				e.ReturnValue = lastPeriod.EndDate;
			}
		}
		}

		protected virtual void Vendor_TaxYearStartDate_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			Vendor vend = e.Row as Vendor;
			if (vend == null) return;
			if (vend.EnableTaxStartDate == true)
			{
			e.NewValue = generateDatesList().Where(date => date.Day == 1).First();
		}
		}
		...
		{
			if (viewName == "PaymentDetails" && DefLocation.Current == null)
			{ 
		...
		protected override void LocationExtAddress_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
		{
			...
			e.Cancel = !ValidateLocation(cache, location);
		}

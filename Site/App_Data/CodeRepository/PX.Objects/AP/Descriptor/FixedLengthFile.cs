			...
			if (records.Count > 0)
			{
				foreach (object record in records)
				{
					...
					foreach (PropertyInfo pInfo in propertyInfo)
					{
						try
						{
							FixedLengthAttribute[] attributes = pInfo.GetCustomAttributes(typeof(FixedLengthAttribute), false).Cast<FixedLengthAttribute>().ToArray();
							FixedLengthAttribute ffa = attributes.FirstOrDefault();
							if(ffa == null) continue;

							string outputString = Convert.ToString(pInfo.GetValue(record, null));
						...
						{
							throw new PXException("Error while writting " + record.GetType().Name + " record. Writing information about " + pInfo.Name + ". Errors : " + ex.Message);
						}
					}
